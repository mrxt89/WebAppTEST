...
const config = {
  server: {
    ...
    ssl: {
      certPath: process.env.SSL_CERT_PATH || path.join(__dirname, 'ssl', 'certificato.pem'),
      keyPath: process.env.SSL_KEY_PATH || path.join(__dirname, 'ssl', 'chiave.pem')
    }
  },
  database: {
  ...
  storage: {
    ...
    remotePath: process.env.REMOTE_STORAGE_PATH || '//192.168.42.121/crite/GestDoc2/WebApp',
    remoteType: process.env.REMOTE_STORAGE_TYPE || 'mounted', // 'mounted', 'smb', 'nfs'
    smb: {
      path: process.env.SMB_PATH || '//192.168.42.121/crite',
      domain: process.env.STORAGE_DOMAIN || 'crite.locale',
      username: process.env.STORAGE_USERNAME || 'rosset',
      password: process.env.STORAGE_PASSWORD || 'OraetLabora25-!'
    },
    nfs: {
...
console.log('Storage Path:', process.env.STORAGE_TYPE === 'remote' ? process.env.REMOTE_STORAGE_PATH : process.env.LOCAL_STORAGE_PATH);
console.log('HTTPS Enabled:', config.server.useHttps);
