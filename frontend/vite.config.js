...
function loadHostConfig(env) {
  ...
  return {
    ...
    apiHttpsPort: parseInt(
      env.VITE_API_HTTPS_PORT || defaults.API_HTTPS_PORT.toString(),
    ),
    apiHttpPort: parseInt(
      env.VITE_API_HTTP_PORT || defaults.API_HTTP_PORT.toString(),
    ),
  };
}
...
export default defineConfig(({ mode }) => {
  ...
  const { hostIp, useHttps, httpsPort, httpPort, apiHttpsPort, apiHttpPort } =
    hostConfig;
        ...
        rollupOptions: {
          output: {
            ...
            manualChunks: {
              ...
              "ui-components": ["@/components/ui", "@/lib/utils"],
            },
