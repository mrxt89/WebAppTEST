...
const useProjectArticlesActions = () => {
    ...
    const fetchItemStatuses = useCallback(async () => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/statuses`);
            if (data) {
    ...
    const fetchItems = useCallback(async (page = 0, pageSize = 50, filters = {}) => {
        try {
            setLoading(true);
...
const getItemById = useCallback(async (itemId) => {
    ...
    try {
        setLoading(true);
        const response = await makeRequest(`${config.API_BASE_URL}/projectArticles/items/${itemId}`)
        ...
        if (typeof response !== 'object' || !('Id' in response)) {
            console.warn(`API returned invalid data structure for item #${itemId}:`, response);
            return null;
        }
    ...
    const addItem = useCallback(async (itemData, projectId) => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items`, {
                method: 'POST',
                body: JSON.stringify({
                    action: 'ADD',
                    itemData,
    ...
    const copyItem = useCallback(async (itemData, projectId, sourceItemId) => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items`, {
                method: 'POST',
    ...
    const getBOMData = async (action, id, itemId = null, version = null, options = {}) => {
        try {
            ...
            if (options.includeDisabled !== undefined) url += `&includeDisabled=${options.includeDisabled}`;
            if (options.expandPhantoms !== undefined) url += `&expandPhantoms=${options.expandPhantoms}`;
            if (options.includeRouting !== undefined) url += `&includeRouting=${options.includeRouting}`;
    ...
    const getBOMByItemId = useCallback(async (itemId, version = null, action = 'GET_BOM_FULL', options = {}) => {
        try {
          if (!itemId) {
          ...
          if (data && !data.header && !data.components && !Array.isArray(data.components)) {
    ...
    const addUpdateBOM = useCallback(async (action, bomData) => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/boms`, {
                method: 'POST',
                body: JSON.stringify({
                    action,
                    bomData
    ...
    const copyBOMFromItem = useCallback(async (projectId, targetItemId, sourceItemId = null, sourceType = 'temporary') => {
        try {
            setLoading(true);
            ...
            if (sourceItemId && sourceType === 'temporary') {
                ...
                if (sourceBom && sourceBom.header) {
                    ...
                    return { success: 0, msg: 'L\'articolo sorgente non ha una distinta base' };
                }
            ...
            const response = await makeRequest(`${config.API_BASE_URL}/projectArticles/boms`, {
                method: 'POST',
                body: JSON.stringify({
                    action: action,
                    bomData: bomData
            ...
            return { success: 0, msg: err.message || 'Errore nella copia della distinta base' };
        } finally {
            setLoading(false);
        }
    ...
    const getERPBOMs = useCallback(async (search = '') => {
        try {
            setLoading(true);
    ...
    const getReferenceBOMs = useCallback(async (filters = {}, page = 1, pageSize = 10) => {
        try {
            setLoading(true);
            ...
            if (filters.onlyAvailable !== undefined) params.append('onlyAvailable', filters.onlyAvailable);
    ...
    const reorderBOMComponents = useCallback(async (bomId, components) => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/boms/${bomId}/reorder`, {
                method: 'POST',
    ...
    const manageReferences = useCallback(async (action, referenceData) => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/references`, {
                method: 'POST',
                body: JSON.stringify({
                    action,
                    referenceData
    ...
    const getItemReferences = useCallback(async (companyId, itemId) => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/references/item/${companyId}/${itemId}`);
            return data;
        } catch (err) {
            setError(err.message);
...
const addComponent = useCallback(async (bomId, componentData = null) => {
    try {
        ...
        if (data.createTempComponent) {
            ...
            if (!data.ComponentId && !data.ComponentCode) {
                throw new Error('ComponentId or ComponentCode required for the ADD_COMPONENT action');
            }
    ...
    const updateComponent = useCallback(async (bomId, line, componentData) => {
...
const addRouting = useCallback(async (bomId, routingData) => {
    try {
      setLoading(true);
    ...
    const deleteRouting = useCallback(async (bomId, rtgStep) => {
        try {
          setLoading(true);
    ...
    const getNatureDescription = useCallback((natureCode) => {
        switch (parseInt(natureCode)) {
            case 22413312: return 'Semilavorato';
            case 22413313: return 'Prodotto Finito';
            case 22413314: return 'Acquisto';
            default: return 'Altro';
        }
    }, []);

    // Helper per tradurre stati articoli
    const getStatusDescription = useCallback((statusId) => {
    ...
    const getAvailableItems = useCallback(async (projectId) => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items/available?projectId=${projectId}`);
            return data || [];
        } catch (err) {
            setError(err.message);
    ...
    const getERPItems = useCallback(async (search = '') => {
        try {
            setLoading(true);
...
const importERPItem = useCallback(async (projectId, erpItem, importBOM = false, processMultilevelBOM = true, maxLevels = 10) => {
    try {
        setLoading(true);
        const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items/import`, {
            method: 'POST',
            body: JSON.stringify({
                action: 'IMPORT',
                projectId,
    ...
    const linkItemToProject = useCallback(async (projectId, itemId) => {
        try {
            setLoading(true);
            const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items/link`, {
                method: 'POST',
                body: JSON.stringify({
                    action: 'LINK',
                    projectId,
...
const replaceComponent = useCallback(async (bomId, componentLine, newComponentId = 0, newComponentCode = '') => {
    try {
        setLoading(true);
        const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/boms/${bomId}/replaceComponent`, {
            method: 'POST',
            body: JSON.stringify({
                componentLine,
                newComponentId,
...
const replaceWithNewComponent = useCallback(async (bomId, componentLine, newComponentData) => {
    try {
        setLoading(true);
        ...
        if (newComponentData.createTempComponent) {
            ...
            payload.newComponentData = {
                Description: newComponentData.Description || "Componente temporaneo",
        ...
        const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/boms/${bomId}/replaceWithNewComponent`, {
            method: 'POST',
...
const unlinkItemFromProject = useCallback(async (projectId, itemId) => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items/${itemId}/project/${projectId}`, {
        method: 'DELETE'
      });
      return data;
    } catch (err) {
  ...
  const disableTemporaryItem = useCallback(async (itemId) => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items/${itemId}/disable`, {
        method: 'PUT'
      });
      return data;
    } catch (err) {
  ...
  const canDisableItem = useCallback(async (itemId) => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items/${itemId}/canDisable`);
      return data;
    } catch (err) {
      setError(err.message);
...
const getWorkCenters = useCallback(async () => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/routing/workcenters`);
      return data || [];
    } catch (err) {
      setError(err.message);
  ...
  const getOperations = useCallback(async () => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/routing/operations`);
      return data || [];
    } catch (err) {
      setError(err.message);
  ...
  const getSuppliers = useCallback(async () => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/routing/suppliers`);
      return data || [];
    } catch (err) {
      setError(err.message);
...
const getBOMVersions = useCallback(async (itemId) => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/boms/versions/${itemId}`);
      return data || [];
    } catch (err) {
      setError(err.message);
...
const reorderBOMRoutings = useCallback(async (bomId, cycles) => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/boms/${bomId}/reorderRoutings`, {
        method: 'POST',
...
const getUnitsOfMeasure = useCallback(async () => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/unitsOfMeasure`);
      return data || [];
    } catch (err) {
      setError(err.message);
  ...
  const updateItemDetails = useCallback(async (itemId, itemData) => {
    try {
      setLoading(true);
      const data = await makeRequest(`${config.API_BASE_URL}/projectArticles/items/${itemId}/details`, {
        method: 'PUT',
