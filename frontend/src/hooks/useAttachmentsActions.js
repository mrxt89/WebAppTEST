...
const useAttachmentsActions = () => {
    ...
    const getItemCodeAttachments = async (itemCode) => {
        ...
        try {
            ...
            const response = await fetch(`${config.API_BASE_URL}/attachments/itemCode`, {
                method: 'POST',
                headers: {
    ...
    const uploadAttachmentLegacy = async (projectId, taskId = 0, file, articleCode = null) => {
        if (articleCode) {
    ...
    const downloadAllAttachments = async (options = {}) => {
        ...
        try {
            ...
            if (itemCode) {
                ...
                fileName = taskId ? `Task_${taskId}_Attachments.zip` : `Project_${projectId}_Attachments.zip`;
            }
