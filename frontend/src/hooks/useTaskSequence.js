...
const useTaskSequence = (initialTasks, updateTaskSequence, getProjectById) => {
  ...
  useEffect(() => {
    ...
    const sortedTasks = [...initialTasks].sort((a, b) => a.TaskSequence - b.TaskSequence);
  ...
  const handleTaskSequenceUpdate = useCallback(async (taskId, projectId, dropIndex) => {
    try {
      setIsUpdating(true);
      ...
      if (result && result.success) {
        ...
        try {
          ...
          console.error('useTaskSequence: Errore nell\'aggiornamento del progetto:', refreshError);
        }
      }
      
      return result || { success: false };
    } catch (error) {
      console.error('useTaskSequence: Errore nell\'aggiornamento della sequenza:', error);
      throw error;
    } finally {
  ...
  const updateTasks = useCallback((newTasks) => {
    if (!isUpdating && newTasks && newTasks.length > 0) {
      const sortedNewTasks = [...newTasks].sort((a, b) => a.TaskSequence - b.TaskSequence);
