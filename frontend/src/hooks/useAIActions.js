...
const useAIActions = () => {
  ...
  const checkAIStatus = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.error || 'Errore nel controllo dello stato dell\'AI');
      }
    } catch (error) {
      console.error('Error checking AI status:', error);
      setError(error.message || 'Si è verificato un errore durante il controllo dello stato dell\'AI');
      return null;
    } finally {
      setLoading(false);
    }
   ...
   */
  const generateConversationSummary = useCallback(async (notificationId, userId) => {
    try {
      setLoading(true);
      setError(null);
      ...
      return {
        ...
        savedHighlights: response.data.savedHighlights || response.data.highlights || [] // Tutti i punti salvati
      };
    } catch (error) {
      console.error('Error in generateConversationSummary:', error);
      setError(error.message || 'Si è verificato un errore durante la generazione del riepilogo');
      // Mostra un messaggio di errore all'utente
      swal.fire({
        title: 'Errore',
        text: error.message || 'Impossibile generare il riepilogo della conversazione',
        icon: 'error',
        timer: 3000
      });
      return {
  ...
  const analyzeSentiment = useCallback(async (text) => {
    try {
      ...
      if (!response.data.success) {
        throw new Error(response.data.error || 'Errore nell\'analisi del sentiment');
      }
      
      return response.data.sentiment;
    } catch (error) {
      console.error('Error in analyzeSentiment:', error);
      setError(error.message || 'Si è verificato un errore durante l\'analisi del sentiment');
      return null;
    } finally {
      setLoading(false);
    }
  ...
  const suggestReply = useCallback(async (notificationId, lastMessage) => {
    try {
      ...
      if (!response.data.success) {
        throw new Error(response.data.error || 'Errore nella generazione della risposta suggerita');
      }
      
      return response.data.suggestedReply;
    } catch (error) {
      console.error('Error in suggestReply:', error);
      setError(error.message || 'Si è verificato un errore durante la generazione della risposta suggerita');
      return '';
    } finally {
      setLoading(false);
    }
