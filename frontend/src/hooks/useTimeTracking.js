...
const useTimeTracking = () => {
  ...
  const isUserAdmin = useCallback(() => {
    ...
    const isAdmin = userGroups.some(g => g.groupName.toUpperCase() === 'ADMIN' || g.groupName.toUpperCase() === 'RESPONSABILI PROGETTI');
    return isAdmin;
  }, [userGroups]);
   ...
   */
  const canViewUserData = useCallback((targetUserId) => {
   ...
   */
  const getUserTimeWeekly = useCallback(async (userId, weekStartDate) => {
    try {
      // Verifica dei permessi
      if (!canViewUserData(userId)) {
        throw new Error('Non hai i permessi per visualizzare i dati di questo utente');
      }
   ...
   * @throws {Error} Se l'utente non ha i permessi necessari
   */
  const getUserAvailableTasks = useCallback(async (userId) => {
    try {
      // Verifica dei permessi
      if (!canViewUserData(userId)) {
        throw new Error('Non hai i permessi per visualizzare i dati di questo utente');
  ...
  const addTimeEntry = useCallback(async (entryData) => {
    try {
      ...
      if (entryData.UserID !== user?.userId && !isUserAdmin()) {
        throw new Error('Non hai i permessi per registrare ore per questo utente');
      }
   ...
   * @returns {Promise<Object>} Risultato dell'operazione
   */
  const updateTimeEntry = useCallback(async (entryId, entryData) => {
    try {
      setLoading(true);
   ...
   */
  const getProjectTimeSummary = useCallback(async (projectId) => {
    try {
      setLoading(true);
   ...
   */
  const getTaskTimeSummary = useCallback(async (taskId) => {
    try {
      setLoading(true);
   ...
   */
  const getMonthlyTimeReport = useCallback(async (userId, month) => {
    try {
      setLoading(true);
   ...
   */
  const exportTimeReport = useCallback(async (userId, period, value, format = 'csv') => {
    try {
      setLoading(true);
