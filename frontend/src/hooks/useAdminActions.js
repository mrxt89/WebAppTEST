...
const useAdminActions = () => {
  ...
  const getUserCompanies = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${config.API_BASE_URL}/user-companies/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const assignUserToCompany = async (userId, companyId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${config.API_BASE_URL}/user/${userId}/assign-company/${companyId}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const removeUserFromCompany = async (userId, companyId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${config.API_BASE_URL}/user/${userId}/remove-company/${companyId}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchUsers(); // Aggiorna l'elenco degli utenti
      return response.data;
  ...
  const setPrimaryCompany = async (userId, companyId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${config.API_BASE_URL}/user/${userId}/set-primary-company/${companyId}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchUsers(); // Aggiorna l'elenco degli utenti
      return response.data;
  ...
  const resetPassword = async (userId, newPassword) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${config.API_BASE_URL}/reset-password`, { userId, newPassword }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchUsers();
    } catch (error) {
  ...
  const toggleUserStatus = async (userId, userDisabled) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${config.API_BASE_URL}/user/${userId}/status`, { userDisabled }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchUsers();
    } catch (error) {
  ...
  const assignUserToGroup = async (userId, groupId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${config.API_BASE_URL}/groups/${groupId}/add-user`, { userId }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchGroups();
    } catch (error) {
  ...
  const removeUserFromGroup = async (userId, groupId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${config.API_BASE_URL}/groups/${groupId}/remove-user`, { userId }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchGroups();
    } catch (error) {
  ...
  const fetchPages = async () => {
    try {
      ...
      const pagesData = response.data.map(page => ({
        ...page,
        groups: typeof page.groups === 'string' ? JSON.parse(page.groups) : (page.groups || []),
        childPages: page.childPages ? page.childPages.split(',').map(id => parseInt(id, 10)) : []
      }));
  ...
  const enableDisablePage = async (pageId, disabled) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${config.API_BASE_URL}/pages/${pageId}/status`, { disabled }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchPages();
    } catch (error) {
  ...
  const toggleInheritPermissions = async (pageId, inheritPermissions) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${config.API_BASE_URL}/pages/${pageId}/inheritance`, { inheritPermissions }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchPages();
    } catch (error) {
  ...
  const assignGroupToPage = async (pageId, groupId, applyToChildren = false) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${config.API_BASE_URL}/pages/${pageId}/add-group`, { groupId, applyToChildren }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchPages();
    } catch (error) {
  ...
  const removeGroupFromPage = async (pageId, groupId, applyToChildren = false) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${config.API_BASE_URL}/pages/${pageId}/remove-group`, { groupId, applyToChildren }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchPages();
    } catch (error) {
  ...
  const fetchNotificationsChannels = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${config.API_BASE_URL}/notifications-channels`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const updateNotificationChannel = async (channelData) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${config.API_BASE_URL}/notifications-channels/${channelData.notificationCategoryId}`, channelData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchNotificationsChannels(); 
    } catch (error) {
  ...
  const addUserToChannel = async (userId, notificationCategoryId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${config.API_BASE_URL}/notifications-channels/${notificationCategoryId}/add-user`, { userId }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const removeUserFromChannel = async (userId, notificationCategoryId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${config.API_BASE_URL}/notifications-channels/${notificationCategoryId}/remove-user`, { userId }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchNotificationsChannels(); // Aggiorna i canali di notifica
    } catch (error) {
