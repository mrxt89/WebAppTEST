...
const useItemAttachmentsActions = () => {
   ...
   */
  const canModifyAttachment = useCallback(
    (attachment) => {
   ...
   */
  const getAttachmentsByItemCode = useCallback(
    async (itemCode, includeShared = true, isErpAttachment = null) => {
      try {
        setLoading(true);
        ...
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/item-code/${itemCode}?${query.toString()}`,
        );
   ...
   * @returns {Promise<Array>} - Lista degli allegati
   */
  const getAttachmentsByProjectItemId = useCallback(
    async (projectItemId, includeShared = true, isErpAttachment = null) => {
      try {
        setLoading(true);
        const query = new URLSearchParams();
        if (includeShared !== undefined)
        ...
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/project-item/${projectItemId}?${query.toString()}`,
        );
   ...
   */
  const uploadAttachmentByItemCode = useCallback(
    async (itemCode, file, metadata = {}) => {
      try {
        setLoading(true);
        ...
        if (metadata.isErpAttachment !== undefined)
          formData.append(
            "isErpAttachment",
            metadata.isErpAttachment.toString(),
          );
        if (metadata.isVisible !== undefined)
          formData.append("isVisible", metadata.isVisible.toString());
        if (metadata.categoryIds)
          formData.append("categoryIds", metadata.categoryIds);
        if (metadata.tags) formData.append("tags", metadata.tags);

        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/item-code/${itemCode}/upload`,
          {
            method: "POST",
            body: formData,
   ...
   * @returns {Promise<Object>} - Dati dell'allegato caricato
   */
  const uploadAttachmentByProjectItemId = useCallback(
    async (projectItemId, file, metadata = {}) => {
      try {
        setLoading(true);
        const formData = new FormData();
        ...
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/project-item/${projectItemId}/upload`,
          {
            method: "POST",
            body: formData,
            // Non includere Content-Type, il browser lo imposterà automaticamente con boundary
    ...
    async (attachmentId, fileName) => {
      try {
        ...
        const response = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}/download`,
          {
            responseType: "blob",
          },
        );
   ...
   */
  const getItemAttachmentById = useCallback(
    async (attachmentId) => {
      try {
        setLoading(true);
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}`,
        );
        return data;
      } catch (err) {
        setError(err.message);
  ...
  const deleteAttachment = useCallback(
    async (attachmentId, hardDelete = false) => {
      try {
        setLoading(true);
        ...
        await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}?hardDelete=${hardDelete}`,
          {
            method: "DELETE",
          },
        );
  ...
  const restoreAttachment = useCallback(
    async (attachmentId) => {
      try {
        ...
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}/restore`,
          {
            method: "POST",
            body: JSON.stringify({}),
          },
  ...
  const shareAttachment = useCallback(
    async (attachmentId, targetCompanyId, accessLevel = "read") => {
      try {
        setLoading(true);
        ...
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}/share`,
          {
            method: "POST",
            body: JSON.stringify({
              targetCompanyId,
   ...
   */
  const unshareAttachment = useCallback(
    async (attachmentId, targetCompanyId) => {
      try {
        setLoading(true);
        ...
        if (attachment && !canModifyAttachment(attachment)) {
          showError(
            "Non hai i permessi per rimuovere la condivisione di questo allegato",
          );
          return false;
        }

        await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}/share/${targetCompanyId}`,
          {
            method: "DELETE",
          },
        );
   ...
   */
  const getAttachmentSharing = useCallback(
    async (attachmentId) => {
      try {
        setLoading(true);
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}/sharing`,
        );
  ...
  const getAttachmentCategories = useCallback(async () => {
    try {
      setLoading(true);
      const data = await makeRequest(
        `${config.API_BASE_URL}/item-attachment-categories`,
      );
   ...
   */
  const addAttachmentCategory = useCallback(
    async (categoryName, description = null, colorHex = "#1b263b") => {
      try {
        setLoading(true);
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachment-categories`,
  ...
  const getAttachmentsByCategory = useCallback(
    async (categoryId) => {
      try {
        setLoading(true);
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachment-categories/${categoryId}/attachments`,
        );
   ...
   */
  const setAttachmentCategories = useCallback(
    async (attachmentId, categoryIds) => {
      try {
        setLoading(true);
        ...
        if (attachment && !canModifyAttachment(attachment)) {
          showError(
            "Non hai i permessi per modificare le categorie di questo allegato",
          );
          return null;
        }

        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}/categories`,
          {
            method: "POST",
            body: JSON.stringify({
   ...
   */
  const getAttachmentVersions = useCallback(
    async (attachmentId) => {
      try {
        setLoading(true);
        const data = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}/versions`,
        );
   ...
   */
  const addAttachmentVersion = useCallback(
    async (attachmentId, file, changeNotes = null) => {
      try {
        setLoading(true);
        ...
        if (attachment && !canModifyAttachment(attachment)) {
          showError(
            "Non hai i permessi per aggiungere versioni a questo allegato",
          );
          return null;
        }
   ...
   */
  const updateAttachmentCodeMapping = useCallback(
    async (attachmentId, oldItemCode, newItemCode) => {
      try {
        setLoading(true);
        ...
        if (attachment && !canModifyAttachment(attachment)) {
          showError(
            "Non hai i permessi per modificare il codice articolo di questo allegato",
          );
          return false;
        }

        await makeRequest(
          `${config.API_BASE_URL}/item-attachments/${attachmentId}/code-mapping`,
          {
            method: "PUT",
            body: JSON.stringify({
              oldItemCode,
   ...
   * @returns {Promise<boolean>} - true se il download è riuscito
   */
  const downloadAllAttachmentsByItemCode = useCallback(
    async (itemCode) => {
      try {
        setLoading(true);
        const response = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/item-code/${itemCode}/download-all`,
          {
            responseType: "blob",
          },
        );
   ...
   * @returns {Promise<boolean>} - true se il download è riuscito
   */
  const downloadAllAttachmentsByProjectItemId = useCallback(
    async (projectItemId) => {
      try {
        setLoading(true);
        const response = await makeRequest(
          `${config.API_BASE_URL}/item-attachments/project-item/${projectItemId}/download-all`,
          {
            responseType: "blob",
          },
        );
        ...
        link.setAttribute(
          "download",
          `ProjectItem_${projectItemId}_Attachments.zip`,
        );
        document.body.appendChild(link);
        link.click();
        link.remove();
        ...
        setError(err.message);
        console.error(
          "Error downloading all attachments by project item ID:",
          err,
        );
        showError("Errore nel download degli allegati");
        throw err;
