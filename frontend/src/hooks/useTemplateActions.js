...
const useTemplateActions = () => {
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/projectsTemplates/templates`, {
        headers: {
    ...
    const fetchFilteredTemplates = useCallback(async (categoryId, detailLine = null) => {
        try {
          setLoading(true);
          setError(null);
    ...
    const addUpdateTemplateDetail = async (templateDetailData) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.API_BASE_URL}/projectsTemplates/templates/details`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const toggleTemplateStatus = async (templateId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.API_BASE_URL}/projectsTemplates/templates/${templateId}/toggle`, {
                method: 'PATCH',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const deleteTemplateDetail = async (templateDetailId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.API_BASE_URL}/projectsTemplates/templates/details/${templateDetailId}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
