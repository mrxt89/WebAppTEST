...
const fetchProjects = useCallback(async (page = 0, pageSize = 50, filters = {}) => {
    try {
      setLoading(true);
    ...
    const addUpdateProject = useCallback(async (projectData) => {
      try {
          const cleanedData = {
              ...
              ProjectCategoryDetailLine: parseInt(projectData.ProjectCategoryDetailLine) || 0,
              ...
              TemplateID: projectData.TemplateID ? parseInt(projectData.TemplateID) : null, // Aggiunto TemplateID
          };
    ...
    const updateProjectMembers = async (projectId, members) => {
        try {
            ...
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.API_BASE_URL}/projects/${projectId}/members`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
  ...
  // Aggiungi o aggiorna task
    const addUpdateProjectTask = async (taskData) => {
        try {
            ...
            if (taskData.TaskID && !checkAdminPermission(project) && !isOwnTask(taskData)) {
            ...
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.API_BASE_URL}/projects/${cleanTaskData.ProjectID}/tasks`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const updateTaskStatus = async (taskData) => {
        try {
            ...
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.API_BASE_URL}/projects/tasks/${taskData.TaskID}/status`, {
                method: 'PATCH',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const addTaskComment = async (taskId, comment) => {
      try {
          const token = localStorage.getItem('token');
          const response = await fetch(`${config.API_BASE_URL}/projects/tasks/${taskId}/comments`, {
              method: 'POST',
              headers: {
                  Authorization: `Bearer ${token}`,
                  'Content-Type': 'application/json',
    ...
    const getUserProjectStatistics = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.API_BASE_URL}/projects/statistics/user`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
      ...
      const getTaskHistory = async (taskId) => {
        try {
            const response = await fetch(`${config.API_BASE_URL}/projects/tasks/${taskId}/history`, {
                headers: {
      ...
      const getTaskCosts = async (taskId) => {
        try {
          const response = await fetch(`${config.API_BASE_URL}/projects/tasks/${taskId}/costs`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
              'Content-Type': 'application/json',
            },
      ...
      const addCost = async (taskId, costData) => {
        ...
        const updateCost = async (taskId, lineId, costData) => {
          try {
            setLoading(true);
            const token = localStorage.getItem('token');
            const response = await fetch(`${config.API_BASE_URL}/projects/tasks/${taskId}/costs/${lineId}`, {
              method: 'PUT',
              headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/json',
        ...
        const handleCalendarEvent = async (taskId, participants) => {
          ...
          const updateTaskSequence = async (taskId, projectId, newSequence) => {
            try {
              ...
              console.error('updateTaskSequence: Errore nell\'aggiornamento sequenza:', error);
              throw error;
            }
          };
          ...
          const getUserTasks = async () => {
            try {
              setLoading(true);
              const token = localStorage.getItem('token');
              const response = await fetch(`${config.API_BASE_URL}/projects/tasks/user`, {
                headers: {
                  Authorization: `Bearer ${token}`,
                  'Content-Type': 'application/json',
                },
