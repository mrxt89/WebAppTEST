...
const useProjectCustomersActions = () => {
  ...
  const searchErpCustomers = useCallback(async (searchText) => {
    try {
      setLoadingErp(true);
  ...
  const getProjectCustomerById = useCallback(async (id) => {
    try {
      const url = `${config.API_BASE_URL}/projectCustomers/${encodeURIComponent(id)}`;
      return await makeRequest(url);
    } catch (error) {
      console.error(`Errore nel caricamento del cliente progetto con ID ${id}:`, error);
      if (!error.message.includes('aborted')) {
        throw error;
      }
    }
  ...
  const updateProjectCustomer = useCallback(async (id, customerData) => {
    try {
      ...
      console.error(`Errore nell'${id ? 'aggiornamento' : 'inserimento'} del cliente progetto:`, error);
      if (!error.message.includes('aborted')) {
        throw error;
      }
    }
  ...
  const toggleDisableProjectCustomer = useCallback(async (id) => {
    try {
      const url = `${config.API_BASE_URL}/projectCustomers/${encodeURIComponent(id)}`;
      const result = await makeRequest(url, {
        method: 'DELETE'
      ...
      console.error('Errore nella modifica dello stato di disabilitazione:', error);
      if (!error.message.includes('aborted')) {
        throw error;
      }
    }
  ...
  const linkToErpCustomer = useCallback(async (id, erpCustSupp) => {
    try {
  ...
  const importProjectCustomers = useCallback(async (customersData) => {
    try {
...
const importFromERP = useCallback(async (erpCustSupp) => {
    try {
