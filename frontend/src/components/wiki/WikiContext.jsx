...
export const WikiProvider = ({ children }) => {
  ...
  const [openedFromNotificationSidebar, setOpenedFromNotificationSidebar] =
    useState(false);
    ...
    const tryNavigation = async () => {
      try {
        ...
        if (success) {
          ...
          console.warn(
            "Impossibile navigare alla visualizzazione richiesta dopo diversi tentativi.",
          );
          return false;
        }
      } catch (error) {
  ...
  const loadWikiContent = async (path, specificSection = null) => {
    ...
    try {
          ...
          switch (tabId) {
            ...
            case "project-tasks-tab":
              ...
              const tasksViewToggler =
                document.getElementById("project-tasks-view");
              if (tasksViewToggler) {
                const viewMode =
                  tasksViewToggler.getAttribute("data-view-mode");
                if (viewMode === "kanban") {
      ...
      try {
        ...
        if (navigateCallback) {
          ...
          try {
            ...
            if (!success) {
              console.warn(
                "Non è stato possibile navigare alla visualizzazione richiesta.",
              );
            }
          } catch (error) {
            console.error("Errore durante la navigazione:", error);
        ...
        console.error(
          `Errore nel caricamento del file ${contentFile}.json:`,
          error,
        );
            ...
            sections: [
              {
                ...
                content:
                  "<p>La guida per questa pagina non è ancora disponibile. Stiamo lavorando per aggiungerla al più presto!</p>",
              },
            ],
          });
        }
      ...
      console.error(
        "Errore generale nel caricamento del contenuto wiki:",
        error,
      );
      // Carica un contenuto di fallback generico
      setCurrentWikiContent({
        title: "Guida Applicazione",
        description:
          "Si è verificato un errore durante il caricamento della guida.",
        sections: [
          {
            title: "Errore",
            content:
              "<p>Si è verificato un errore durante il caricamento della guida. Riprova più tardi.</p>",
          },
        ],
      });
    }
  ...
  const startTour = (tourName) => {
    if (
      ...
      !currentWikiContent.tours[tourName]
    ) {
...
export const useWikiContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useWikiContext deve essere usato all'interno di un WikiProvider",
    );
  }
  return context;
};
