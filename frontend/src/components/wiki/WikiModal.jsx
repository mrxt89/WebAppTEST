...
const WikiModal = () => {
  ...
  useEffect(() => {
    if (isWikiOpen && currentWikiContent && currentWikiContent._navigateCallback) {
  ...
  const filteredSections = searchQuery 
    ? currentWikiContent.sections.filter(section => 
  ...
  const hasTours = currentWikiContent.tours && Object.keys(currentWikiContent.tours).length > 0;
  ...
  return (
    <Dialog 
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-xl md:text-2xl">{currentWikiContent.title || 'Guida Applicazione'}</DialogTitle>
          <DialogDescription className="text-base">
            {currentWikiContent.description || 'Benvenuto nella documentazione interattiva della piattaforma.'}
          </DialogDescription>
          
        </DialogHeader>
        {/* Tabs */}
        <Tabs defaultValue="guida" value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="space-y-4 pr-4">
                  {filteredSections.map((section, index) => (
                    <div key={index} className="border rounded-md p-4 hover:shadow-md transition-shadow">
                      ...
                      {section.tourId && currentWikiContent.tours && currentWikiContent.tours[section.tourId] && (
                        <Button 
                          variant="outline" 
            ...
            {hasTours ? (
              <ScrollArea className="h-[calc(100vh-15rem)]">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pr-4">
                  {Object.entries(currentWikiContent.tours).map(([tourId, tourSteps], index) => (
                    <Card key={index} className="hover:shadow-md transition-shadow">
                      <CardContent className="p-5">
                        ...
                        <p className="text-gray-600 mb-3">
                          {tourSteps[0]?.description || `Un tour guidato con ${tourSteps.length} passaggi`}
                        </p>
                        <Button onClick={() => startTour(tourId)} className="w-full justify-between">
                          Inizia il tour
        ...
        <DialogFooter className="border-t pt-4 mt-4">
          <Button variant="outline" onClick={handleCloseWiki}>Chiudi</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
