...
import { Download, Trash2, Upload, Camera, File, Image, FileText } from 'lucide-react';
...
const AttachmentsList = ({ notificationId, onAttachmentUploaded }) => {
  ...
  const handleFileChange = async (e) => {
    if (e.target.files && e.target.files[0]) {
      try {
        ...
        swal.fire('Errore', 'Si è verificato un errore durante il caricamento.', 'error');
      } finally {
  ...
  const handleCapturePhoto = async () => {
    try {
      ...
      swal.fire('Errore', 'Si è verificato un errore durante la cattura della foto.', 'error');
    }
  };
