...
const PopoutButton = ({ notificationId, title, onSuccess }) => {
  ...
  const { registerStandaloneChat, isStandaloneChat, openChatInNewWindow } =
    useNotifications();
  ...
  const handlePopout = useCallback(async () => {
    ...
    try {
      ...
      if (isStandaloneChat && isStandaloneChat(notificationId)) {
        ...
        console.warn(
          `Finestra esistente non trovata, creazione nuova finestra`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Il browser ha bloccato l'apertura della nuova finestra",
        );
      }
