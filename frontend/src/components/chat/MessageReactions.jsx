...
const MessageReactions = ({ messageId, notificationId, onReactionUpdated }) => {
  ...
  const { getMessageReactions, toggleMessageReaction, removeMessageReaction } =
    useNotifications();
    ...
    try {
      ...
      if (result && Array.isArray(result)) {
        ...
        const hasChanges =
          ...
          JSON.stringify(currentReactionsRef.current);
  ...
  useEffect(() => {
    const handleReactionUpdate = (event) => {
      ...
      if (
        ...
        (eventNotificationId && eventNotificationId === notificationId)
      ) {
  ...
  const handleReactionClick = async (reactionType, userReactionId = null) => {
    try {
      ...
      if (userReactionId) {
        ...
        const userHasAnyReaction = Object.values(
          groupReactionsByType(reactions),
        ).some((reactorGroup) =>
          reactorGroup.some((reactor) => reactor.UserID === currentUserId),
        );
