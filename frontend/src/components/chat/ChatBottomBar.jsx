...
const ChatBottomBar = ({  
  ...
  // Use functions from props first, and fall back to context if needed
  const contextFunctions = useNotifications();
  const sendNotificationWithAttachments = props => contextFunctions.sendNotificationWithAttachments;
  const sendNotification = props => contextFunctions.sendNotification;
...
}, [disabled, inputRef.current]); // Dipendenze per rieseguire l'effetto
...
const handleSendWithAttachments = async () => {
  ...
  if (message.trim() || attachments.length > 0) {
    ...
    try {
      ...
      // Usa la funzione sendNotificationWithAttachments dal context o dalle props
      const sendFunc = typeof onSend === 'function' ? onSend : contextFunctions.sendNotificationWithAttachments;
      
      result = await sendFunc(notificationData, attachments);
  ...
  if (msg.trim()) {
    ...
    try {
      ...
      if (typeof onSend === 'function') {
        ...
        result = await contextFunctions.sendNotification(notificationData);
      }
