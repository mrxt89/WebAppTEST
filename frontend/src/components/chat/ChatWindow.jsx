...
const ChatWindow = ({
  ...
  const updateMessagesFromNotification = useCallback(() => {
    ...
    try {
      ...
      const hasNewMessagesReceived =
        ...
        (lastMessage &&
          (!lastMessageIdRef.current ||
            lastMessage.messageId !== lastMessageIdRef.current)); // ID diverso
      ...
      setHasLeftChat(
        updatedNotification.chatLeft === 1 ||
          updatedNotification.chatLeft === true,
      );
      setIsArchived(
        updatedNotification.archived === 1 ||
          updatedNotification.archived === true,
      );
  ...
  useEffect(() => {
    ...
    if (
      ...
      const hasNewMessage =
        notification.lastUpdated !== lastNotificationRef.current.lastUpdated;
