      ...
      {attachments.map((attachment) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500">{Math.round(attachment.FileSizeKB / 1024 * 100) / 100} MB</p>
          </div>
          <button 
...
const ModernChatList = ({ 
  ...
  useEffect(() => {
    if (currentNotification) {
      ...
      if (!userHasScrolledRef.current && chatListRef.current && !initialScrollDone) {
...
useEffect(() => {
    ...
    getNotificationAttachments(notificationId)
      .then(attachments => {
        if (attachments) {
          ...
          setMessageAttachments(prev => ({...prev, [notificationId]: attachmentsByMessageId}));
        }
      })
      .catch(error => {
  ...
  const scheduleReactionsFetch = useCallback(debounce(() => {
    if (visibleMessagesRef.current.size === 0) return;
    
    const messageIds = Array.from(visibleMessagesRef.current)
      .filter(id => !messageReactionsCache[id] && !pendingReactionsRequestsRef.current[id]);
  ...
  }, 300), [messageReactionsCache]);

  // Carica le reazioni in batch
  const batchLoadReactions = useCallback(async (messageIds) => {
    if (!messageIds || messageIds.length === 0 || !getMessageReactions) return;
    
    const unrequestedIds = messageIds.filter(id => 
      ...
      try {
        ...
        for (let i = 0; i < unrequestedIds.length; i += maxParallelRequests) {
          ...
          const responses = await Promise.all(
            batch.map(messageId => 
              axios.get(`${config.API_BASE_URL}/messages/${messageId}/reactions`, {
                headers: { Authorization: `Bearer ${token}` }
              })
            )
          );
  ...
  useEffect(() => {
    ...
    document.addEventListener('message-reaction-updated', handleMessageReactionUpdated);
    
    return () => {
      document.removeEventListener('message-reaction-updated', handleMessageReactionUpdated);
    };
  }, [scheduleReactionsFetch]);

  // Funzione per caricare i sondaggi
  const loadPolls = useCallback(debounce(async () => {
        ...
        localMessages.forEach(message => {
          if (isPollMessage(message)) {
            if (message.pollId) {
              ...
              let matchingPoll = allPolls.find(p => p.MessageID === message.messageId);
              
              if (!matchingPoll && message.message) {
                const titleMatch = message.message.match(/\*\*Sondaggio creato\*\*: "([^"]+)"/);
                if (titleMatch && titleMatch[1]) {
                  const pollTitle = titleMatch[1];
                  matchingPoll = allPolls.find(p => p.Question === pollTitle);
                }
              }
  ...
  }, 300), [notificationId, localMessages, getNotificationPolls]);
  ...
  useEffect(() => {
    if (notificationId) {
      ...
      const hasPollMessages = localMessages.some(message => isPollMessage(message));
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (editingMessageId !== null && !event.target.closest('.message-menu') && !event.target.closest('.message-actions-menu')) {
        setEditingMessageId(null);
      }
      
      if (avatarContextMenu !== null && !event.target.closest('.avatar-context-menu')) {
        setAvatarContextMenu(null);
      }
      
      if (colorPickerMessageId !== null && !event.target.closest('.color-picker-area')) {
...
const handleViewVersionHistory = async (messageId) => {
  ...
  const handleDeleteMessage = async (messageId) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        document.dispatchEvent(new CustomEvent('message-deleted', { 
          detail: { 
            messageId, 
        ...
        throw new Error(response.data?.message || 'Errore durante l\'eliminazione del messaggio');
      }
    } catch (error) {
      ...
      swal.fire({
        icon: 'error',
        title: 'Errore',
        text: error.message || 'Si è verificato un errore durante l\'eliminazione del messaggio'
      });
    }
  };
  ...
  const handleColorButtonClick = (messageId, event) => {
    ...
    event.stopPropagation();
    setColorPickerMessageId(messageId === colorPickerMessageId ? null : messageId);
    setEditingMessageId(null);
  };
...
const handleReactionSelect = async (messageId, emoji) => {
  ...
  const findOriginalMessage = (replyToMessageId) => {
    return localMessages.find((message) => message.messageId == replyToMessageId);
  };
  ...
  const renderEditedIndicator = (message, isCurrentUserMessage) => {
    ...
    return (
      <div className={`text-xs ${isCurrentUserMessage == '1' ? 'text-white/70' : 'text-gray-400'} mt-1 flex items-center`}>
        <Clock className="h-3 w-3 mr-1" />
        <span>
          Modificato{message.lastEditDate ? ` ${formatTimeAgo(message.lastEditDate)}` : ''}
        ...
        <div className="flex flex-col">
          <div className="flex items-center p-2 border-b border-gray-200">
            <div 
              ...
              style={{ backgroundColor: generateAvatarColor(`${user.firstName} ${user.lastName}`) }}
            >
              {user.firstName[0]}{user.lastName[0]}
            </div>
            <div className="ml-2">
              <div className="text-sm font-medium">{user.firstName} {user.lastName}</div>
              <div className="text-xs text-gray-500">{user.email || 'Nessuna email'}</div>
            </div>
          </div>
  ...
  const readByUsers = localMessages.map((message) => {
    let users = [];
    try {
      if (typeof message.readedByUsers === 'string' && message.readedByUsers.trim() !== '') {
        let sanitizedData = message.readedByUsers.trim().replace(/[\u0000-\u001F\u007F-\u009F]/g, "");
      ...
      console.error('Error parsing readByUsers JSON:', e, message.readedByUsers);
    }
    return users;
  });
                          ...
                          <div className="flex flex-col max-w-[70%]">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {isCurrentUserMessage == '1' && (
                                <div className={cn(
                                  'message-timestamp',
                                  isCurrentUserMessage == '0' ? 'text-right' : 'text-left'
                                )}>
                                  {formatTime(message.tbCreated)}
                                  {message.isEdited == '1' && <span className="ml-1 text-gray-400 text-[10px]">✎</span>}
                                </div>
                              ...
                              <div
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setEditingMessageId(editingMessageId === message.messageId ? null : message.messageId);
                                    }}
                                  ...
                                  {/* Color picker */}
                                  {colorPickerMessageId === message.messageId && !hasLeftChat && (
                                    <div className={`absolute z-50 ${isCurrentUserMessage == '1' ? 'right-10' : 'left-10'} top-2 flex ${isCurrentUserMessage == '1' ? 'flex-row-reverse' : 'flex-row'} items-start color-picker-area`}>
                                      <MessageColorPicker 
                                        messageId={message.messageId} 
                                        onClose={() => setColorPickerMessageId(null)} 
                                  ...
                                  {/* Indicatore di messaggi letti */}
                                  {isCurrentUserMessage == '1' && readers?.length > 0 && (
                                    <div 
                                      className="absolute bottom-1 right-2 text-xs text-gray-500 cursor-pointer"
                                      onClick={() => handleInfoClick(message.messageId)}
                                    >
                                  ...
                                  {renderEditedIndicator(message, isCurrentUserMessage)}
                                </div>
