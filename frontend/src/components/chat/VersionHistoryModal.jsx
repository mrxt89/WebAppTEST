...
const VersionHistoryModal = ({
  ...
  const getCurrentMessageText = () => {
    ...
    return (
      ...
      (typeof versionData.currentMessage === "string"
  ...
  const getEditorName = (version) => {
    ...
    return version.EditedByName || version.editorName || "Utente";
  };
  ...
  const getVersionEditDate = (version) => {
    ...
    return version.EditedDate || version.editDate || null;
  };
              ...
              {currentMessageText && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {currentEditDate && (
                      <div className="text-xs text-gray-500 flex items-center">
                        ...
                        <span>
                          {formatDate(currentEditDate)}{" "}
                          {formatTime(currentEditDate)}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="version-card current-version-card p-4 rounded-lg border">
                    ...
                    {versionData.currentMessage &&
                      versionData.currentMessage.editCount > 0 && (
                        <div className="mt-2 text-xs text-blue-500">
                          Modificato {versionData.currentMessage.editCount}{" "}
                          {versionData.currentMessage.editCount === 1
                            ? "volta"
                            : "volte"}
                        </div>
                      )}
              ...
              {versions.length > 0 ? (
                  ...
                  <div className="space-y-6">
                    {versions.map((version, index) => (
                      <div
                        ...
                        className="space-y-2 animated-fade-in"
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {getVersionEditDate(version) && (
                            <div className="text-xs text-gray-500 flex items-center">
                              <Clock className="h-3.5 w-3.5 mr-1.5" />
                              <span>
                                {formatDate(getVersionEditDate(version))}{" "}
                                {formatTime(getVersionEditDate(version))}
                              </span>
                            </div>
                          )}
                        </div>
                ...
                <div className="text-center py-8 bg-gray-50 rounded-lg border border-gray-200 mt-4">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    Questo messaggio non è stato modificato in precedenza
                  </p>
                </div>
              )}
            </div>
