...
import { Search, Filter, FileText, Package, Users, File, Link, MessageSquare, Info } from 'lucide-react';
        ...
        <div className="p-4">
          ...
          <div className="border rounded-md overflow-hidden mb-4">
            <div className="bg-gray-50 px-4 py-2 text-sm font-medium text-gray-700 border-b flex justify-between items-center">
              <span>Risultati</span>
              <span className="text-xs text-gray-500">{searchResults.length} chat trovate</span>
            </div>
            ...
            <div className="max-h-[400px] overflow-y-auto">
              {loading ? (
                ...
                <div className="p-4 text-center text-gray-500">
                  {searchType !== 'all' || searchValue ? 'Nessun risultato trovato' : 'Inserisci criteri di ricerca per trovare le chat'}
                </div>
              ) : (
                <div className="divide-y divide-gray-200">
                  {searchResults.map((chat) => (
                    <div 
                      ...
                      onClick={() => handleChatClick(chat.notificationId, chat.isUserMember)}
                    >
                      <div className="flex items-start space-x-3">
                        <div className="flex-shrink-0">
                          <div className="p-2 rounded-full" style={{ backgroundColor: chat.hexColor || '#e5e7eb' }}>
                            ...
                            <div className="flex flex-wrap gap-1">
                              ...
                              {chat.CustSuppCode && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded bg-yellow-100 text-yellow-800">
                                  {chat.CustSuppType === 3211265 ? "Cliente" : "Fornitore"}: {chat.CustSuppCode}
                                </span>
                              )}
                            </div>
                          </div>
