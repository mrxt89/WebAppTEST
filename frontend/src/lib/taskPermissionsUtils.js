...
export const hasAdminOrManagerPermission = (project, userId) => {
    ...
    return userMember && (userMember.Role === 'ADMIN' || userMember.Role === 'MANAGER');
  };
  ...
  export const canEditMemberRole = (project, currentUserId, targetUserId) => {
    ...
    if (!project || !project.members || !currentUserId || !targetUserId) return false;
    
    // Get current user's role
    const currentUserMember = project.members.find(m => m.UserID === parseInt(currentUserId));
    
    // Get target user's role
    const targetUserMember = project.members.find(m => m.UserID === parseInt(targetUserId));
