...
const MyTasksPage = () => {
  ...
  const applyFilters = useCallback((tasksList, mainFilters, colFilters = {}) => {
    ...
    if (mainFilters.priority !== 'all') {
      result = result.filter(task => task.Priority === mainFilters.priority);
    }
    ...
    if (mainFilters.projectId !== 'all') {
      result = result.filter(task => task.ProjectID.toString() === mainFilters.projectId);
    }
    ...
    if (colFilters.participants && isUserAdmin) {
      ...
      result = result.filter(task => {
        ...
        try {
          ...
          return participants.some(p => 
            `${p.firstName} ${p.lastName}`.toLowerCase().includes(participantsFilter) ||
    ...
    const sortDirection = colFilters.sortDirection || mainFilters.sortDirection || 'asc';
  ...
  const handleTaskUpdate = async (taskData, shouldCloseModal = false) => {
    try {
      ...
      if (result.success) {
        ...
        setTasks(prev => 
          prev.map(t => t.TaskID === completeTaskData.TaskID ? { ...t, ...completeTaskData } : t)
        );
        
        // Aggiorna i filtri applicati
        applyFilters(tasks.map(t => 
  ...
  return (
    <div className="p-6 mx-auto space-y-6">
      ...
      {selectedTask && (
        <TaskDetailsDialog
          project={{ ProjectID: selectedTask.ProjectID, AdminPermission: selectedTask.AdminPermission }}
          task={selectedTask}
