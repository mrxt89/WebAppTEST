...
const MyTasksTimelineView = ({
  ...
  const isTaskDueThisWeek = (task) => {
    ...
    return (
      dueDate > today && dueDate <= endOfWeek && task.Status !== "COMPLETATA"
    );
  };
  ...
  const isTaskDueThisMonth = (task) => {
    ...
    return (
      dueDate > today && dueDate <= endOfMonth && task.Status !== "COMPLETATA"
    );
  };
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-0 sm:justify-between mb-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isTaskDelayed(task) && (
              <Badge
                variant=""
                className="flex items-center gap-1 bg-red-100 text-red-500 border-red-200 text-xs"
              >
          ...
          <Badge
            className={`${priorityColors[task.Priority]} text-xs sm:text-sm whitespace-nowrap`}
          >
            {task.Priority}
          </Badge>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-between text-xs sm:text-sm">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-1 min-w-0">
              <Clock
                className={`w-3 h-3 sm:w-4 sm:h-4 shrink-0 ${isTaskDelayed(task) ? "text-red-400" : "text-gray-400"}`}
              />
              <span
                className={`${isTaskDelayed(task) ? "text-red-400 font-medium" : "text-gray-500"}`}
              >
                {task.DueDate
          ...
          return (
            ...
            >
              <AccordionTrigger className="flex items-center justify-between px-4 py-3 hover:no-underline [&[data-state=open]>div]:text-opacity-100 [&[data-state=closed]>div]:text-opacity-75">
                <div
                  className={`flex items-center gap-3 w-full ${timelineConfig[timeline].headerColor}`}
                >
