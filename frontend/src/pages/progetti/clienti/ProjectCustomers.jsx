...
const ProjectCustomers = () => {
    ...
    try {
      ...
      const { value: formValues } = await swal.fire({
        ...
        didOpen: () => {
          ...
          regionSelect.addEventListener("change", () => {
            ...
            if (!countrySelect.value && regionSelect.value) {
              const match = countriesData.find(
                (item) => item.Region === regionSelect.value,
              );
          ...
          countySelect.addEventListener("change", () => {
            if (
              countySelect.value &&
              (!countrySelect.value || !regionSelect.value)
            ) {
      ...
      if (formValues) {
        ...
        if (result && result.success) {
          ...
          throw new Error(
            result?.message || "Errore nell'aggiunta del cliente",
          );
        }
      }
    } catch (error) {
      ...
      if (searchTerm) {
        ...
        if (selectedErpCustSupp) {
          ...
          if (result && result.success) {
            ...
            throw new Error(
              result?.message || "Errore nel collegamento al cliente ERP",
            );
          }
        }
      }
          ...
          try {
            ...
            if (isConfirmed) {
              ...
              if (result && result.success) {
                ...
                throw new Error(
                  result?.message || "Errore durante l'importazione",
                );
              }
            }
          } catch (error) {
      ...
      if (searchTerm) {
        ...
        if (selectedErpCustSupp) {
          ...
          if (result && result.success) {
            ...
            throw new Error(
              result?.message || "Errore nell'importazione del cliente",
            );
          }
        }
      }
  ...
  return (
    <div
      className="p-3"
      style={{ maxWidth: "98%", justifyContent: "center", margin: "auto" }}
    >
