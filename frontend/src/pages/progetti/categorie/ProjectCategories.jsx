  ...
  const handleSaveSubcategory = async () => {
    try {
      if (!currentSubcategory?.Description) {
        swal.fire(
          "Attenzione",
          "Il nome della sottocategoria è obbligatorio",
          "warning",
        );
        return;
      }
      ...
      const result = await addUpdateSubcategory({
        ProjectCategoryId: currentCategory.ProjectCategoryId,
        Line:
          currentSubcategory.Line || (currentCategory.details?.length || 0) + 1,
      ...
      swal.fire(
        "Errore",
        "Errore nel salvataggio della sottocategoria",
        "error",
      );
    }
  };
  ...
  const handleToggleSubcategory = async (categoryId, line) => {
    try {
      const subcategory = categories
        .find((c) => c.ProjectCategoryId === categoryId)
        ?.details.find((d) => d.Line === line);
      if (!subcategory) {
    ...
    <div className="p-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {category.details?.map((detail) => (
                          <Badge
                            ...
                            className={{
                              "bg-red-100": detail.Disabled,
                              "bg-green-100": !detail.Disabled,
                              "text-red-500": detail.Disabled,
                              "text-black-500": !detail.Disabled,
                            }}
                          >
                            {detail.Description}
                            <button
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          size="icon"
                          onClick={() =>
                            handleToggleCategory(category.ProjectCategoryId)
                          }
                        >
                          {category.Disabled ? (
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {currentCategory?.ProjectCategoryId
                ? "Modifica Categoria"
                : "Nuova Categoria"}
            </DialogTitle>
          </DialogHeader>
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-4 pt-4">
            ...
            <Button className="w-full" onClick={handleSaveSubcategory}>
              Salva
            </Button>
          </div>
        </DialogContent>
