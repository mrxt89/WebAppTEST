...
const CollapsibleFilters = ({
  ...
  const dialog = React.Children.toArray(children).find(
    (child) => React.isValidElement(child) && child.type === Dialog,
  );

  const filterElements = React.Children.toArray(children).filter(
    (child) => React.isValidElement(child) && child.type !== Dialog,
  );
      ...
      <Card className="overflow-hidden bg-white/50 backdrop-blur-sm border-none shadow-sm">
        <CardHeader className="p-3">
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <Filter
                    className={`h-4 w-4 ${hasActiveFilters ? "text-blue-500" : "text-gray-400"}`}
                  />
              ...
              <div className="flex items-center gap-2">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="bottom">
                    {isExpanded ? "Comprimi filtri" : "Espandi filtri"}
                  </TooltipContent>
                </Tooltip>
                {/* New Project button */}
                <div className="shrink-0 ml-2">{dialog}</div>
              </div>
            </div>
            ...
            >
              <div className="overflow-hidden">{filterElements}</div>
            </div>
          </div>
        </CardHeader>
      </Card>
