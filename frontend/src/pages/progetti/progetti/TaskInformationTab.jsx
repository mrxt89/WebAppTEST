...
const TaskInformationTab = ({
  ...
  const filteredAssignableUsers = assignableUsers.filter(user => {
    ...
    if (showSelectedOnly) {
      return isNotLeader && selectedParticipants.includes(user.userId.toString());
    }
    return isNotLeader;
  });
  ...
  const getParticipantDetails = () => {
    ...
    return selectedParticipants
      .map(participantId => {
        const user = assignableUsers.find(u => u.userId.toString() === participantId);
        return user ? {
          userId: user.userId,
    ...
    >
          ...
          <div className="flex items-center gap-1">
            ...
            {isEditing ? (
              <Input
                ...
                onChange={(e) => setEditedData(prev => ({ ...prev, StartDate: e.target.value }))}
                className="w-40"
              />
            ) : (
        ...
        <div className="space-y-2 w-50">
          ...
          {isEditing ? (
            <Select
              ...
              onValueChange={(value) => setEditedData(prev => ({ ...prev, AssignedTo: value }))}
            >
              <SelectTrigger>
            ...
            <div className="flex items-center gap-2">
              <Avatar className="h-8 w-8">
                <AvatarFallback>
                  {getInitials(task?.AssignedToFirstName, task?.AssignedToLastName)}
                </AvatarFallback>
              </Avatar>
              ...
              <div className="p-2">
                {filteredAssignableUsers.map(user => (
                  <div key={user.userId} className="flex items-center space-x-2 hover:bg-gray-100 p-2 rounded">
                    <Checkbox
                      checked={selectedParticipants.includes(user.userId.toString())}
                      onCheckedChange={(checked) => {
                        setSelectedParticipants(prev => {
                          if (checked) {
                            return [...prev, user.userId.toString()];
                          }
                          return prev.filter(id => id !== user.userId.toString());
                        });
                      }}
                      id={`user-${user.userId}`}
                      className="data-[state=checked]:bg-blue-500 data-[state=checked]:border-blue-500"
                    />
                    <Label htmlFor={`user-${user.userId}`} className="flex-grow cursor-pointer text-sm">
                      {user.firstName} {user.lastName}
                    </Label>
                  </div>
                ))}
            ...
            <div className="flex flex-wrap gap-2">
              {getParticipantDetails().length > 0 ? (
                getParticipantDetails().map((participant, index) => (
                  <Badge key={index} variant="" className="flex items-center gap-2 bg-gray-100 text-black">
      ...
      <div className="space-y-2">
        ...
        {isEditing ? (
                  ...
                  <div className="flex flex-col gap-1 py-1">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className={
                        ...
                        t.Status === 'BLOCCATA' ? 'bg-red-100 text-red-700' :
                        t.Status === 'SOSPESA' ? 'bg-yellow-100 text-yellow-700' :
          ...
          <div className="flex items-center gap-2">
            <Badge variant="outline" className={
              predecessorTask.Status === 'COMPLETATA' ? 'bg-green-100' : 
              predecessorTask.Status === 'IN ESECUZIONE' ? 'bg-blue-100' : 
              'bg-gray-100'
            }>
          ...
          <span className="text-sm text-gray-500"> Nessuna attività collegata</span>
        )}
      </div>
