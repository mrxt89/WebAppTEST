...
import { Calendar, AlertTriangle, MessageSquare, Paperclip, CheckCircle2, Loader2, ListTodo, AlertCircle, LockIcon } from 'lucide-react';
...
const UpdatedTaskRow = ({ 
      ...
      if (editingCell.field === 'AssignedTo') {
        ...
        try {
          if (task.Participants) {
            participants = typeof task.Participants === 'string' 
              ? JSON.parse(task.Participants) 
              : task.Participants;
            participants = participants.filter(p => p.userId.toString() !== task.AssignedTo.toString());
          }
      ...
      onTaskUpdate(updatedTask).then(result => {
        if (result && result.success) {
          toast({
            ...
            description: `${editingCell.field === 'Status' ? 'Stato' : 
                        editingCell.field === 'AssignedTo' ? 'Responsabile' : 
                        editingCell.field === 'Priority' ? 'Priorità' : 
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        toast({
          title: "Aggiornamento riuscito",
          description: `${editingCell.field === 'Title' ? 'Titolo' : 
                      editingCell.field === 'Status' ? 'Stato' : 
                      editingCell.field === 'AssignedTo' ? 'Responsabile' : 
                      editingCell.field === 'Priority' ? 'Priorità' : 
                      editingCell.field === 'DueDate' ? 'Scadenza' : 
                      'Campo'} aggiornato con successo`,
          variant: "success",
          duration: 2000,
          style: { backgroundColor: '#2c7a7b', color: '#fff' }
  ...
  const renderEditableOrLocked = (field, content, editableContent, locked = !canEditField(field)) => {
    return (
  ...
  return (
    ...
    >
      ...
      {/* Responsabile */}
      <TableCell>
        {editingCell.taskId === task.TaskID && editingCell.field === 'AssignedTo' ? (
          <Select
          ...
          renderEditableOrLocked(
            ...
            <div 
              ...
              onClick={(e) => handleCellClick(e, 'AssignedTo', task.AssignedTo?.toString() || '')}
            >
      ...
      {/* Stato */}
      <TableCell>
        {editingCell.taskId === task.TaskID && editingCell.field === 'Status' ? (
          <Select
            value={editValue}
            onValueChange={handleSelectChange}
            onClick={(e) => e.stopPropagation()}
      ...
      {/* Scadenza */}
      <TableCell>
        {editingCell.taskId === task.TaskID && editingCell.field === 'DueDate' ? (
          <Input
            type="date"
            value={editValue}
            onChange={handleInputChange}
      ...
      {/* Priorità */}
      <TableCell>
        {editingCell.taskId === task.TaskID && editingCell.field === 'Priority' ? (
          <Select
            value={editValue}
            onValueChange={handleSelectChange}
            onClick={(e) => e.stopPropagation()}
      ...
      <TableCell className="text-center">
        {task.CommentsCount > 0 ? (
          <Badge variant="outline" className="flex items-center justify-center mx-auto gap-1 bg-blue-50 text-blue-600">
