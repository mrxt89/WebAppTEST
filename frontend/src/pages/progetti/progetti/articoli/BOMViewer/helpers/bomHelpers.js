...
export const buildBOMTree = (components, routing = []) => {
  if (!Array.isArray(components) || components.length === 0) {
    console.warn("buildBOMTree called with invalid or empty components array", components);
    return [];
  }
  
  // Debug ID for tracing function execution
  const debugId = Date.now().toString().slice(-6);
  console.log(`[buildTree-${debugId}] Starting tree build with ${components ? components.length : 0} components and ${routing ? routing.length : 0} routing entries`);
  ...
  componentsToProcess.forEach(comp => {
    ...
    comp.ComponentId = comp.ComponentId || comp.ItemId || `unknown-${Math.random().toString(36).substring(2)}`;
    ...
    if (comp.Path) {
      ...
      if (parentNode) {
        ...
        for (const [id, node] of Object.entries(nodeMap)) {
          if (node.data.ComponentId === comp.ComponentId && node.data.Line === comp.Line && 
        ...
        if (nodeToAdd) {
          ...
          if (parentNode.data.BOMId && !nodeToAdd.data.BOMId) {
            ...
            console.log(`[buildTree-${debugId}] Propagated BOMId ${parentNode.data.BOMId} to child ${nodeToAdd.id}`);
          }
    ...
    // Otherwise, try to infer parent from level
      ...
      for (let i = componentsToProcess.indexOf(comp) - 1; i >= 0; i--) {
        ...
        if (potentialParent.Level === comp.Level - 1) {
          ...
          let parentNode = null;
          for (const [id, node] of Object.entries(nodeMap)) {
            if (node.data.ComponentId === potentialParent.ComponentId && node.data.Line === potentialParent.Line) {
              parentNode = node;
              break;
            }
          }
          ...
          if (parentNode && childNode) {
            ...
            if (parentNode.data.BOMId && !childNode.data.BOMId) {
              ...
              console.log(`[buildTree-${debugId}] Propagated BOMId ${parentNode.data.BOMId} to child ${childNode.id}`);
            }
  ...
  const sortNodeChildren = (node) => {
    if (node.children && node.children.length > 0) {
      ...
      node.children.sort((a, b) => {
        ...
        const aLine = a.type === 'component' ? (a.data.Line || 0) : (a.data.RtgStep || 0);
        const bLine = b.type === 'component' ? (b.data.Line || 0) : (b.data.RtgStep || 0);
        return aLine - bLine;
      });
  ...
  const nodesWithBOMId = Object.values(nodeMap).filter(node => node.data.BOMId).length;
  const totalNodes = Object.values(nodeMap).length;
  console.log(`[buildTree-${debugId}] BOMId propagation stats: ${nodesWithBOMId}/${totalNodes} nodes have BOMId (${Math.round(nodesWithBOMId/totalNodes*100)}%)`);
  ...
  export const calculateBOMSummary = (bom, components = []) => {
    ...
    const purchased = Array.isArray(components) 
      ? components.filter(c => (c.Nature || c.ComponentNature) === 22413314).length 
      : 0;
    
    const semifinished = Array.isArray(components) 
      ? components.filter(c => (c.Nature || c.ComponentNature) === 22413312).length 
      : 0;
    
    const finished = Array.isArray(components) 
      ? components.filter(c => (c.Nature || c.ComponentNature) === 22413313).length 
      : 0;
...
export const hasPendingChanges = (pendingChanges) => {
  ...
  for (const key in pendingChanges) {
    ...
    if (key.startsWith('cycle-')) {
      ...
      if (changes.cycleChanges && Object.keys(changes.cycleChanges).length > 0) {
        return true;
      }
 ...
 */
export const resetPendingChanges = (pendingChanges, setPendingChanges, componentId = null) => {
  if (componentId) {
...
export const hasComponentChanges = (pendingChanges, componentId) => {
  ...
  if (
    ...
    (
      (pendingChanges[componentId].bomComponentChanges && 
       Object.keys(pendingChanges[componentId].bomComponentChanges).length > 0) ||
  ...
  if (pendingChanges[cycleKey]) {
    ...
    if (
      ...
      (cycleChanges.cycleChanges && Object.keys(cycleChanges.cycleChanges).length > 0) ||
