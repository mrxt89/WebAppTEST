...
const BOMTreeView = ({
  ...
  const buildComponentsTree = useCallback((components, routing = []) => {
    ...
    if (!Array.isArray(components) || components.length === 0) {
      console.warn(
        `[buildTree-${debugId}] No components or invalid components array`,
      );
      return [];
    }
    ...
    componentsToProcess.forEach((comp, index) => {
      ...
      if (comp.Level === undefined || comp.Level === null) {
        console.warn(
          `[buildTree-${debugId}] Component at index ${index} has no Level, setting default`,
        );
      ...
      if (!comp.ComponentId) {
        if (comp.ItemId) {
          console.warn(
            `[buildTree-${debugId}] Component at index ${index} has no ComponentId, using ItemId (${comp.ItemId})`,
          );
          comp.ComponentId = comp.ItemId;
        } else {
          console.warn(
            `[buildTree-${debugId}] Component at index ${index} has no ComponentId or ItemId, generating unique id`,
          );
      ...
      if (comp.Line === undefined || comp.Line === null) {
        console.warn(
          `[buildTree-${debugId}] Component at index ${index} has no Line, setting to index`,
        );
        comp.Line = index;
      }
        ...
        if (comp.Level === 0 || comp.Level === 1) {
          ...
          for (let i = index - 1; i >= 0; i--) {
            if (componentsToProcess[i].Level === comp.Level - 1) {
              let parentPath =
                componentsToProcess[i].Path ||
                componentsToProcess[i].ComponentId.toString();
          ...
          if (!pathFound) {
            comp.Path = comp.ComponentId.toString();
            console.warn(
              `[buildTree-${debugId}] No parent found for level ${comp.Level} component at index ${index}, using simple path: ${comp.Path}`,
            );
          }
        }
      }
      ...
      comp.ComponentItemCode =
        comp.ComponentItemCode || comp.ComponentCode || "";
    ...
    componentsToProcess.forEach((comp) => {
      ...
      if (!parentFound) {
        ...
        if (!parentFound) {
          console.warn(
            `[buildTree-${debugId}] No parent found for ${nodeId}, adding to root nodes`,
          );
    ...
    if (rootNodes.length === 0 && Object.keys(nodeMap).length > 0) {
      console.warn(
        `[buildTree-${debugId}] No root nodes found, using first component as root`,
      );
  ...
  const fetchDirectData = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "Fetching direct data for BOM ID with projectArticlesActions:",
        selectedBomId,
      );
      ...
      if (data) {
        ...
        if (data.components && Array.isArray(data.components)) {
          ...
          if (typeof data === "object") {
            const arrayProps = Object.keys(data).filter(
    ...
    const loadData = async () => {
      ...
      if (selectedBomId && !initialLoadDoneRef.current) {
        try {
          ...
          setLoadError(
            error.message || "Errore nel caricamento dei dati della distinta",
          );
        ...
        const timeoutId = setTimeout(() => {
          if (selectedBomId) {
            ...
            console.warn(
              "selectedBomId still not set after retry, user may need to click refresh",
            );
          }
        }, 500);
  ...
  useEffect(() => {
    ...
    const componentsToUse =
      bomComponents && bomComponents.length > 0 ? bomComponents : [];
    ...
    try {
      if (item?.bomStato_erp === "1" || item?.bomStato_erp === 1) {
        toast({
          ...
          description:
            "Non è possibile aggiungere componenti a una distinta presente in ERP (Mago)",
          variant: "destructive",
        });
        return;
      }
    ...
    <div className="flex flex-col h-full">
              ...
              <>
                ...
                {editMode && (
                  <Button
                    ...
                    className="mt-4"
                    disabled={
                      item?.bomStato_erp === "1" || item?.bomStato_erp === 1
                    }
                  >
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="py-4 space-y-4">
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-1">
                Tutti i componenti selezionati vengono sostituiti con lo stesso
                codice
              </p>
            </div>
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-1">
                Tutti i componenti selezionati vengono sostituiti con un codice
                esistente
              </p>
            </div>
      ...
      <Dialog open={showManualDialog} onOpenChange={setShowManualDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              Inserisci nuovo codice manuale per tutti i componenti
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setManualData({ ...manualData, code: e.target.value })
                  }
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setManualData({ ...manualData, nature: value })
                  }
                >
              ...
              <div className="col-span-2 flex items-center space-x-2 pt-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setManualData({ ...manualData, copyBOM: checked })
                  }
                  className="h-4 w-4 bg-primary"
                />
                ...
                try {
                  if (componentsToReplace.length > 0) {
                    ...
                    const firstResult = await replaceWithNewComponent(
                      firstComponent.data.ParentBOMId ||
                        firstComponent.data.BOMId,
                    ...
                    if (!firstResult.success) {
                      throw new Error(
                        firstResult.msg || "Errore durante la sostituzione",
                      );
                    }

                    // Ottieni info sul nuovo componente creato
                    const createdCode =
                      firstResult.createdComponentCode || manualData.code;
                    ...
                    if (componentsToReplace.length > 1) {
                      ...
                      if (
                        referenceBOMsResult &&
                        referenceBOMsResult.items &&
                        Array.isArray(referenceBOMsResult.items)
                      ) {
                        const foundItem = referenceBOMsResult.items.find(
                          (b) =>
                            b.ItemCode === manualData.code ||
                            b.ItemCode === createdCode,
                        );
                      ...
                      if (!newComponentId) {
                        ...
                        if (Array.isArray(erpItems)) {
                          const matchingItem = erpItems.find(
                            (item) =>
                              item.Item === manualData.code ||
                              item.Item === createdCode,
                          );
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                        ...
                        onClick={async () => {
                          ...
                          if (componentsToReplace.length === 0) {
                            toast({
                              title: "Nessun componente da sostituire",
                              description:
                                "Tutti i componenti selezionati sono protetti",
                              variant: "warning",
                            });
              ...
              {hasLockedComponents && (
                <div className="mt-2 p-2 bg-amber-50 rounded text-sm flex items-start">
                  <AlertTriangle className="h-4 w-4 text-amber-500 mr-2 mt-0.5 flex-shrink-0" />
                  <div>
                    <strong>Nota:</strong> {lockedCount} componenti sono
                    protetti e non verranno modificati.
                  </div>
                </div>
              )}
            </Tabs>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="py-4 space-y-4">
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  toast({
                    ...
                    description:
                      error.message ||
                      "Si è verificato un errore durante l'aggiunta dei componenti",
                    variant: "destructive",
                  });
                } finally {
                  setIsRefreshing(false);
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-1">
                Crea automaticamente un nuovo codice temporaneo sotto ogni
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-1">
                Inserisci manualmente il codice da aggiungere sotto ogni
                componente selezionato
              </p>
            </div>
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-1">
                Seleziona un codice esistente da aggiungere sotto ogni
                componente selezionato
              </p>
            </div>
      ...
      >
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              Inserisci codice da aggiungere sotto ai componenti selezionati
            </DialogTitle>
          </DialogHeader>
      ...
      >
        <DialogContent className="sm:max-w-[650px]">
          <DialogHeader>
            <DialogTitle>
              Seleziona codice da aggiungere sotto ai componenti selezionati
            </DialogTitle>
          </DialogHeader>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              {hasLockedComponents && (
                <div className="mt-2 p-2 bg-amber-50 rounded text-sm flex items-start">
                  ...
                  <div>
                    <strong>Nota:</strong> {lockedCount} componenti sono
                    protetti e non saranno modificati.
                  </div>
                </div>
              )}
            </Tabs>
