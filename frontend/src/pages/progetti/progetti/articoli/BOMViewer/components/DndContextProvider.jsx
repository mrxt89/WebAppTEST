...
const DndContextProvider = ({ children }) => {
  ...
  const isComponentLocked = (node, operation = "modify") => {
    ...
    if (operation === "addSibling") {
      ...
      if (
        parentComponent &&
        (parentComponent.data.bomStato_erp === "1" ||
          parentComponent.data.bomStato_erp === 1)
      ) {
        return true;
      }
  ...
  const canHaveChildren = (node) => {
    ...
    const nature = parseInt(
      node.data.Nature || node.data.ComponentNature || 0,
      10,
    );
  ...
  const handleDragOver = (event) => {
    ...
    if (
      over.id === "empty-bom-drop-area" &&
      over.data.current &&
      over.data.current.isEmptyBOM
    ) {
    ...
    if (over.data.current && over.data.current.type === "component-node") {
      ...
      if (
        !element ||
        element.getAttribute("data-node-id").startsWith("cycle-")
      ) {
        return;
      }
  ...
  const handleDragEnd = async (event) => {
    ...
    if (over && dropTarget) {
      try {
        ...
        // Double-check it's not a cycle
          ...
          toast({
            ...
            description:
              "Non è possibile eseguire operazioni di drag & drop sui cicli di produzione",
            variant: "warning",
          });
        ...
        // Add under case
          ...
          if (isComponentLocked(dropTarget, "addUnder")) {
            toast({
              title: "Operazione non consentita",
              description:
                "Non è possibile aggiungere componenti sotto questo componente",
              variant: "warning",
            });
          ...
          if (!canHaveChildren(dropTarget)) {
            toast({
              title: "Operazione non consentita",
              description:
                "Non è possibile aggiungere componenti sotto un componente di acquisto",
              variant: "warning",
            });
        ...
        // Add as sibling case
          ...
          if (isComponentLocked(dropTarget, "addSibling")) {
            toast({
              title: "Operazione non consentita",
              description:
                "Non è possibile aggiungere componenti allo stesso livello di questo componente",
              variant: "warning",
            });
          ...
          toast({
            title: "Componente aggiunto",
            description:
              "Componente aggiunto allo stesso livello del componente selezionato",
            variant: "success",
          });
          ...
          toast({
            ...
            description:
              "La modalità di trascinamento non è stata riconosciuta",
            variant: "warning",
          });
        ...
        toast({
          ...
          description:
            error.message ||
            "Si è verificato un errore durante l'operazione di drag and drop",
          variant: "destructive",
        });
      ...
      toast({
        ...
        description:
          "L'area di trascinamento non è valida per questo tipo di elemento",
        variant: "warning",
      });
    }
  ...
  const findParentComponent = (component) => {
    ...
    const componentIndex = bomComponents.findIndex(
      (comp) =>
  ...
  const handleAddSiblingOperation = async (target, draggedItem) => {
    if (!selectedBomId || !target?.data) {
      throw new Error(
        "Dati insufficienti per l'operazione di aggiunta di nodo fratello",
      );
    }
    ...
    if (isRootComponent) {
      ...
      if (
        target.data.parentBOMStato_erp === "1" ||
        target.data.parentBOMStato_erp === 1
      ) {
        toast({
          title: "Errore",
          description:
            "Non puoi aggiungere un componente fratello quando la distinta principale è presente in Mago.",
          variant: "destructive",
        });
        return;
      }
    ...
    // Per componenti non root, verifica solo lo stato del padre
      ...
      if (
        ...
        toast({
          title: "Errore",
          description:
            "Non puoi aggiungere un componente fratello quando il componente padre è presente in Mago.",
          variant: "destructive",
        });
        return;
      }
    ...
    if (createTempComponent) {
      ...
      const description =
        ...
        sourceItem.ComponentDescription ||
      ...
      if (isRootComponent) {
        ...
        parentComponentId = parentComponent
          ? parentComponent.data.ComponentId
          : null;
      }
      ...
      const payload = {
        ...
        sourceComponentId:
          sourceItem.ItemId || sourceItem.Id || sourceItem.ComponentId || 0,
      ...
      const componentCode =
        sourceItem.ItemCode || sourceItem.BOM || sourceItem.Item || "";
  ...
  const handleReplaceOperation = async (target, draggedItem) => {
    ...
    if (isRootComponent) {
      ...
      if (
        ...
        console.error(
          "ERRORE: Controllo parentBOMStato_erp mancato in isComponentLocked",
        );
        toast({
          title: "Errore",
          description:
            "Non puoi sostituire questo componente perché la distinta principale è presente in Mago.",
          variant: "destructive",
        });
        return;
      }
    ...
    // For non-root components, double check both the component and parent states
      ...
      if (target.data.bomStato_erp === "1" || target.data.bomStato_erp === 1) {
        console.error(
          "ERRORE: Controllo bomStato_erp mancato in isComponentLocked",
        );
        toast({
          title: "Errore",
          description:
            "Non puoi sostituire questo componente perché è presente in Mago.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (
        ...
        toast({
          title: "Errore",
          description:
            "Non puoi sostituire questo componente perché il padre è presente in Mago.",
          variant: "destructive",
        });
        return;
      }
    ...
    try {
      ...
      toast({
        ...
        description:
          error.message ||
          "Si è verificato un errore durante la sostituzione del componente",
        variant: "destructive",
      });
      throw error;
    }
  ...
  const handleAddUnderOperation = async (target, draggedItem) => {
    ...
    if (isRootComponent) {
      ...
      if (target.data.bomStato_erp === "1" || target.data.bomStato_erp === 1) {
        toast({
          title: "Errore",
          description:
            "Non puoi aggiungere componenti sotto a una distinta già presente in Mago.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (target.data.bomStato_erp === "1" || target.data.bomStato_erp === 1) {
        toast({
          title: "Errore",
          description:
            "Non puoi aggiungere componenti a una distinta già presente in Mago.",
          variant: "destructive",
        });
        return;
      }
    ...
    const nature = parseInt(
      target.data.Nature || target.data.ComponentNature || 0,
      10,
    );
    if (nature === 22413314) {
      // Purchase component
      toast({
        title: "Errore",
        description:
          "Non puoi aggiungere componenti sotto un componente di acquisto.",
        variant: "destructive",
      });
      return;
    }
  ...
  const setDragSettings = useCallback(
    (settings) => {
      if (activeItem) {
        setActiveItem((prev) => ({
          ...prev,
  ...
  return (
    ...
    >
      ...
      {dropTarget &&
        dropMode &&
        dropTarget.type !== "cycle" &&
        dropTarget.type !== "empty-bom" && (
          <DropIndicator
            target={dropTarget}
            mode={dropMode}
            debugInfo={debugInfo}
