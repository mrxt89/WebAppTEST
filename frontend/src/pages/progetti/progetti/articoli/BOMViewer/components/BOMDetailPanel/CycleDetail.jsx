  ...
  useEffect(() => {
    ...
    const filteredCycles = bomRouting.filter((cycle) => {
      ...
      const bomAndLineMatch =
        ...
        cycle.ComponentLine !== undefined &&
        cycle.ComponentLine == componentLine;
    ...
    console.log(
      `Trovati ${filteredCycles.length} cicli per il componente selezionato`,
    );
    setDebugInfo({
      visible: filteredCycles.length === 0 && bomRouting.length > 0,
      data: debug,
    });
    ...
    try {
      ...
      for (const rtgStep in editedCycles) {
        ...
        const cycleToSave = {
          ...editedCycle,
          ProcessingTime:
            typeof editedCycle.ProcessingTime === "string" &&
            editedCycle.ProcessingTime.includes(":")
              ? parseTimeToSeconds(editedCycle.ProcessingTime)
              : editedCycle.ProcessingTime,
          SetupTime:
            typeof editedCycle.SetupTime === "string" &&
            editedCycle.SetupTime.includes(":")
      ...
      {cycleData.length === 0 ? (
        ...
        <>
          <Table className="border-2">
            <TableHeader>
              <TableRow>
                <TableHead className="w-24 text-center border-r-2">
                  FASE
                </TableHead>
                <TableHead className="w-1/2 text-center border-r-2">
                  OPERAZIONE
                </TableHead>
            ...
            <TableBody>
              {cycleData.map((cycle, index) => (
                <React.Fragment key={cycle.RtgStep}>
                  ...
                  <TableRow className="border-b-0">
                    ...
                    <TableCell className="border-r-2 text-center">
                      {isEditingCycles ? (
                        <Input
                          value={
                            editedCycles[cycle.RtgStep]?.RtgStep ||
                            cycle.RtgStep
                          }
                          onChange={(e) =>
                    ...
                    <TableCell className="border-r-2">
                      {isEditingCycles ? (
                        <>
                          <Select
                            value={
                              editedCycles[cycle.RtgStep]?.Operation || "none"
                            }
                            onValueChange={(value) =>
                              handleCycleFieldChange(
                                cycle.RtgStep,
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              {operations.map((op) => (
                                <SelectItem
                                  key={op.Operation}
                                  value={op.Operation || `op-${op.id}`}
                                >
                                  {op.Description ||
                                    op.Operation ||
                                    "Operazione senza nome"}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                  ...
                  <TableRow
                    className={index < cycleData.length - 1 ? "border-b-4" : ""}
                  >
                    {/* Tasti/Azioni */}
                    <TableCell className="border-r-2 text-center p-1">
                      <div className="text-xs font-semibold mb-1 uppercase text-red-600">
                        Tasti
                      </div>
                      {isEditingCycles && (
                    ...
                    <TableCell className="border-r-2 p-1">
                      <div className="text-xs font-semibold mb-1 uppercase text-amber-600 text-center">
                        Esecuzione
                      </div>
                      {isEditingCycles ? (
                        <Input
                          value={
                            typeof editedCycles[cycle.RtgStep]
                              ?.ProcessingTime === "string" &&
                            editedCycles[
                              cycle.RtgStep
                            ]?.ProcessingTime.includes(":")
                    ...
                    <TableCell className="border-r-2 p-1">
                      ...
                      {isEditingCycles ? (
                        <Input
                          value={
                            typeof editedCycles[cycle.RtgStep]?.SetupTime ===
                              "string" &&
                    ...
                    <TableCell className="p-1">
                      <div className="text-xs font-semibold mb-1 uppercase text-green-700 text-center">
                        Note
                      </div>
                      {isEditingCycles ? (
                        <Textarea
                          value={
                            editedCycles[cycle.RtgStep]?.Notes ||
                            cycle.Notes ||
                            ""
                          }
                          onChange={(e) =>
