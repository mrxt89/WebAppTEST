...
const BOMHeader = () => {
  ...
  const findParentComponent = () => {
    ...
    const index = bomComponents.findIndex(
      (comp) =>
  ...
  const handleVersionChange = async (version) => {
    try {
      ...
      if (!versionData) {
        console.error(
          `Dati per versione ${newVersion} non trovati nella mappa`,
        );
        toast({
          title: "Errore",
  ...
  const handleAddComponentOptions = () => {
    if (!selectedBomId) {
      toast({
        ...
        description:
          "Seleziona una distinta base prima di aggiungere componenti",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (cycleChanges.deletedCycles && cycleChanges.deletedCycles.length > 0) {
        for (const cycle of cycleChanges.deletedCycles) {
          ...
          console.log(
            `Eliminato ciclo RtgStep: ${cycle.RtgStep} da BOM ID: ${bomId}`,
          );
        }
      }
      ...
      if (cycleChanges.newCycles && cycleChanges.newCycles.length > 0) {
        for (const cycle of cycleChanges.newCycles) {
          ...
          console.log(
            `Aggiunto nuovo ciclo RtgStep: ${cycle.RtgStep} a BOM ID: ${bomId}`,
          );
        }
      }
      ...
      if (
        ...
        for (const rtgStep in cycleChanges.cycleChanges) {
          ...
          if (Object.keys(changes).length > 0) {
            ...
            if (
              typeof updatedCycle.ProcessingTime === "string" &&
              updatedCycle.ProcessingTime.includes(":")
            ) {
              updatedCycle.ProcessingTime = parseTimeToSeconds(
                updatedCycle.ProcessingTime,
              );
            }
            if (
              typeof updatedCycle.SetupTime === "string" &&
              updatedCycle.SetupTime.includes(":")
            ) {
              updatedCycle.SetupTime = parseTimeToSeconds(
                updatedCycle.SetupTime,
              );
            }
            ...
            console.log(
              `Aggiornato ciclo RtgStep: ${rtgStep} di BOM ID: ${bomId}`,
            );
          }
        }
      }
      ...
      if (cycleChanges.newOrder && cycleChanges.newOrder.length > 0) {
        ...
        if (cyclesToReorder.length > 0) {
          ...
          console.log(
            `Riordinati ${cyclesToReorder.length} cicli di BOM ID: ${bomId}`,
          );
        }
      }
      ...
      console.error(
        "Errore durante il salvataggio delle modifiche ai cicli:",
        error,
      );
      throw error;
    }
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      if (bom && (bomHasChanged || descriptionHasChanged || statusHasChanged)) {
        ...
        if (!result.success) {
          throw new Error(
            result.msg || "Errore nel salvataggio delle modifiche",
          );
        }
        ...
        if (versionMapRef.current[selectedBomVersion]) {
          ...
          versionMapRef.current[selectedBomVersion].Description =
            bomDescription;
        }
      }
        ...
        for (const componentId in pendingChanges) {
          ...
          if (componentId.startsWith("cycle-")) {
            try {
              ...
              savePromises.push(
                handleSaveCycleChanges(
                  changes,
                  changes.bomId,
                  actualComponentId,
                ).catch((err) => {
                  console.error(
                    `Errore nel salvataggio dei cicli per il componente ${actualComponentId}:`,
                    err,
                  );
                  errors.push(
                    `Errore nel salvataggio dei cicli: ${err.message}`,
                  );
                }),
              );
            } catch (error) {
              console.error(
                `Errore nel salvataggio dei cicli per ${componentId}:`,
                error,
              );
            ...
            try {
              ...
              if (Object.keys(changes.bomComponentChanges).length > 0) {
                ...
                const bomIdToUse =
                  parentComponent && parentComponent.BOMId
                    ? parentComponent.BOMId
                    : changes.bomId;
                ...
                savePromises.push(
                  updateComponent(
                    ...
                    console.error(
                      `Errore nell'aggiornamento del componente ${componentId}:`,
                      err,
                    );
                    errors.push(
                      `Errore nell'aggiornamento del componente: ${err.message}`,
                    );
                  }),
                );
                ...
                console.log("Aggiornamento componente con:", {
                  ...
                  padre: parentComponent
                    ? `${parentComponent.ComponentId} (BOMId: ${parentComponent.BOMId})`
                    : "nessuno",
                });
              }
              ...
              if (Object.keys(changes.itemDetailsChanges).length > 0) {
                ...
                savePromises.push(
                  updateItemDetails(
                    ...
                    console.error(
                      `Errore nell'aggiornamento dei dettagli dell'articolo ${componentId}:`,
                      err,
                    );
                    errors.push(
                      `Errore nell'aggiornamento dei dettagli dell'articolo: ${err.message}`,
                    );
                  }),
                );
              }
              ...
              console.error(
                `Errore nel salvataggio delle modifiche per ${componentId}:`,
                error,
              );
              errors.push(
                `Errore nel salvataggio delle modifiche: ${error.message}`,
              );
            }
          }
        }
      ...
      if (errors.length > 0) {
        ...
        toast({
          ...
          description:
            "Le modifiche alla distinta base sono state salvate con successo",
          variant: "success",
        });
      }
  ...
  const handleCreateVersion = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        throw new Error(
          result.msg || "Errore nella creazione della nuova versione",
        );
      }
    } catch (error) {
      ...
      toast({
        ...
        description:
          error.message ||
          "Si è verificato un errore durante la creazione della nuova versione",
        variant: "destructive",
      });
    } finally {
      setIsCreating(false);
  ...
  const handleCreateNewBOM = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        throw new Error(
          result.msg || "Errore nella creazione della distinta base",
        );
      }
    } catch (error) {
      ...
      toast({
        ...
        description:
          error.message ||
          "Si è verificato un errore durante la creazione della distinta base",
        variant: "destructive",
      });
    } finally {
      setIsCreating(false);
    ...
    return (
      <div className="bg-gray-50 p-6 border-b flex flex-col items-center justify-center">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">
          {item ? item.Item : ""} -{" "}
          {item ? item.Description : "Seleziona un articolo"}
        </h2>
    ...
    <div className="bg-gray-50 p-4 border-b">
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          {editMode ? (
            ...
            <h2
              ...
              title={bom?.Description || item?.Description}
            >
              {bom ? bom.BOM : item?.Item} -{" "}
        ...
        {!editMode && parentComponent && (
          <Badge
            variant="outline"
            className="text-xs bg-blue-50 whitespace-nowrap"
          >
            Padre:{" "}
            {parentComponent.ComponentItemCode || parentComponent.ComponentId}
          </Badge>
        )}
      ...
      <Dialog open={showAddManualDialog} onOpenChange={setShowAddManualDialog}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddWithManual}>Aggiungi</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Dialog per selezione di un codice esistente */}
      <Dialog open={showAddSelectDialog} onOpenChange={setShowAddSelectDialog}>
