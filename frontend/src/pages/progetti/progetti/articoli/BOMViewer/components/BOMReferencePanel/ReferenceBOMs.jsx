...
const ReferenceBOMs = ({ importOptions }) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        const updatedFilter = {
          ...
          nature: natureFilter === 'all' ? '' : 
                  natureFilter === 'semifinished' ? '22413312' :
                  natureFilter === 'finished' ? '22413313' :
                  natureFilter === 'purchased' ? '22413314' : ''
        };
    ...
    loadReferenceBOMs();
  }, [getReferenceBOMs, setReferenceBOMs, setLoading, referenceFilter, pagination.currentPage, natureFilter]);
      ...
      <div className="p-3 border-b">
        ...
        <div className="flex gap-2">
          ...
          <Button 
            variant="outline" 
            size="icon" 
            onClick={() => handleFilterChange('onlyAvailable', !referenceFilter.onlyAvailable)}
            className={referenceFilter.onlyAvailable ? 'bg-blue-50' : ''}
            title={referenceFilter.onlyAvailable ? 'Mostra tutti' : 'Mostra solo disponibili'}
          >
            <Filter className={`h-4 w-4 ${referenceFilter.onlyAvailable ? 'text-blue-500' : ''}`} />
          </Button>
        </div>
      </div>
