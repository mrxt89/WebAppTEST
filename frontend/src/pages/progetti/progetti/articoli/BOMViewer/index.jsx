...
const BOMViewer = ({ item, project, canEdit = false, onRefresh }) => {
  ...
  if (!item?.Id) {
    return (
      <div className="flex items-center justify-center h-full p-8 text-center border rounded-md">
        <div>
          <h3 className="text-lg font-medium mb-2">Nessun articolo selezionato</h3>
          <p className="text-gray-500">Seleziona un articolo per visualizzare la distinta base</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <DndContextProvider>
        {({ draggingOver, dropTarget, dropMode, activeItem, setDragSettings }) => (
