...
export const useBOMRouting = () => {
  ...
  const handleAddRouting = useCallback(async (routingData) => {
    if (!selectedBomId) {
    ...
    try {
      ...
      if (!routingData.RtgStep) {
        const maxStep = bomRouting.reduce((max, step) => 
          step.RtgStep > max ? step.RtgStep : max, 0);
      ...
      toast({
        title: "Error",
        description: error.message || "An error occurred while adding the phase",
        variant: "destructive"
      });
  ...
  const handleUpdateRouting = useCallback(async (rtgStep, routingData) => {
  ...
  const handleDeleteRouting = useCallback(async (rtgStep) => {
    if (!selectedBomId || !rtgStep) {
      console.error('Missing required parameters');
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "An error occurred while deleting the phase",
        variant: "destructive"
      });
  ...
  const handleReorderRouting = useCallback(async (steps) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "An error occurred while reordering phases",
        variant: "destructive"
      });
  ...
  }, [selectedBomId, bomRouting, setBomRouting, setLoading, addUpdateBOM, loadBOMData]);
