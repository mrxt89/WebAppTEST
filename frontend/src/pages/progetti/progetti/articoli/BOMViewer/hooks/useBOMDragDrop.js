...
export const createBOMDragDropHook = (dependencies) => {
  ...
  const handleDrop = useCallback(async (droppedItem) => {
    if (!selectedBomId) {
      toast({
        title: "Error",
  ...
  const reorderComponents = useCallback(async (componentIds) => {
    if (!selectedBomId || !Array.isArray(componentIds) || componentIds.length === 0) {
      return;
    }
    
    // Verify that reorderBOMComponents is defined
    if (!reorderBOMComponents) {
      console.error('reorderBOMComponents function is not defined in context');
      toast({
        title: "Error",
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "An error occurred while reordering components",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
