...
// Provider component
export const BOMViewerProvider = ({ children, item, project, canEdit = false, onRefresh }) => {
  // Import API actions
...
const getBOMVersions = useCallback(async (itemId) => {
  ...
  try {
    ...
    const processedVersions = versions.map(v => ({
      ...v,
      Version: typeof v.Version === 'number' ? v.Version : parseInt(v.Version, 10)
    }));
...
}, [projectArticlesActions, workCenters.length, operations.length, suppliers.length]);
    ...
    try {
      ...
      if (item?.Id && !selectedBomId) {
        ...
        if (data && data.header) {
          ...
          setAvailableVersions(data.availableVersions || [data.header.Version || 1]);
      ...
      if (selectedBomId) {
        ...
        const bomId = typeof selectedBomId === 'object' ? selectedBomId.Id : selectedBomId;
        
        // Incrementa il contatore delle chiamate API
        const requestId = ++apiRequestCount.current;
        
        console.log(`Caricamento dati multilivello per BOM ID: ${bomId}, versione: ${selectedBomVersion}`);
      ...
      if (isMounted.current) {
        toast({
          title: "Error",
          description: "Unable to load BOM data: " + (error.message || "Unknown error"),
          variant: "destructive"
        });
...
const loadBOMData = useCallback(async (action = 'GET_BOM_FULL', options = {}, forceLoad = false) => {
  ...
  const versionToUse = options.version !== undefined ? options.version : selectedBomVersion;
  ...
  if (!forceLoad && 
    ...
    console.log(`Saltato caricamento dati per BOM ID ${bomId}, versione ${versionToUse} - già caricato`);
    return null;
  }
  ...
  try {
    ...
    console.log(`[${requestId}] Caricamento ${action} per BOM ID: ${bomId}, versione: ${versionToUse}`);
    ...
    console.log(`[${requestId}] Risposta per ${action}:`, data ? 'Dati ricevuti' : 'Nessun dato');
    ...
    if (data) {
      ...
      if (action === 'GET_BOM_MULTILEVEL') {
        ...
        if (data.header) {
          ...
          console.log(`[${requestId}] Versione header: ${headerVersion}, richiesta: ${requestedVersion}`);
          ...
          if (headerVersion === requestedVersion) {
            ...
            console.warn(`[${requestId}] Versione nei dati ricevuti (${headerVersion}) non corrisponde a quella richiesta (${requestedVersion})`);
          }
        } else {
...
}, [selectedBomId, projectArticlesActions, selectedBomVersion, toast, setBom, setBomComponents, setBomRouting, setLoading, setSelectedBomVersion, setSelectedBomId]);
  
  // Initial data load
  useEffect(() => {
    const loadInitialData = async () => {
      // Carica solo se abbiamo un BOM ID e non abbiamo già caricato
      if (selectedBomId && !didFetchRef.current && isMounted.current && !loadInProgress.current) {
        ...
        try {
          ...
          console.log("Caricamento dati iniziale con selectedBomVersion:", selectedBomVersion);
...
const getBOMByItemId = useCallback(async (itemId, version = null, action = 'GET_BOM_FULL', options = {}) => {
  ...
  try {
    ...
    const versionToUse = version !== null ? version : selectedBomVersion || 1;
    
    console.log(`Richiesta getBOMByItemId per item ${itemId}, versione ${versionToUse}`);
  ...
  try {
    ...
    if (isMounted.current) {
      ...
      if (currentState.selectedNodeId) {
        ...
        const findNodeInNewData = (nodeId) => {
          ...
          if (parts.length >= 3) {
            ...
            for (const comp of bomComponents) {
              if (comp.ComponentId === parseInt(compId) && comp.Line === parseInt(line)) {
                return {
                  id: nodeId,
                  data: comp
                };
        ...
        const newSelectedNode = findNodeInNewData(currentState.selectedNodeId);
        if (newSelectedNode) {
...
}, [selectedBomId, expandedNodes, selectedNode, activeTab, loadBOMData, item, selectedBomVersion, projectArticlesActions, bomComponents, setBom, didFetchRef]);
...
const updateItemDetails = useCallback(async (itemId, itemData) => {
  ...
  try {
    ...
    const result = await projectArticlesActions.updateItemDetails(itemId, itemData);
    
    if (result.success) {
      // Mostra un messaggio di successo
      toast({
        title: "Dettagli articolo aggiornati",
        description: "I dettagli dell'articolo sono stati aggiornati con successo",
        variant: "success"
      });
      ...
      throw new Error(result.msg || "Errore nell'aggiornamento dei dettagli articolo");
    }
  } catch (error) {
    ...
    if (isMounted.current) {
      toast({
        title: "Errore",
        description: error.message || "Si è verificato un errore durante l'aggiornamento dei dettagli articolo",
        variant: "destructive"
      });
    }
  ...
  const getReferenceBOMs = useCallback(async (filters = {}, page = 1, pageSize = 10) => {
    try {
      const data = await projectArticlesActions.getReferenceBOMs(filters, page, pageSize);
      return data;
    } catch (error) {
      console.error('Error getting reference BOMs:', error);
      return { items: [], pagination: { currentPage: page, totalPages: 1, totalItems: 0 } };
    }
  }, [projectArticlesActions]);
  
  // Handle component action function
const handleComponentAction = useCallback(async (action, data) => {
  if (!selectedBomId) {
    toast({
      title: "Error",
  ...
  try {
    ...
    const result = await projectArticlesActions.addUpdateBOM(action, componentData);
    ...
    if (isMounted.current) {
      toast({
        title: "Error",
        description: error.message || `An error occurred during ${action} operation`,
        variant: "destructive"
      });
    }
    return false;
  ...
  const handleRemoveComponents = useCallback(async (components) => {
    ...
    try {
      ...
      for (const component of components) {
        if (component.data && component.data.Line) {
          ...
          const bomIdToUse = component.data.ParentBOMId || component.data.BOMId || selectedBomId;
          console.log(`Rimozione componente ${component.data.Line} da BOM ID ${bomIdToUse}`);
  ...
  const handleAddComponentBelow = useCallback(async (component) => {
    ...
    try {
      ...
      const newComponentCode = prompt("Inserisci il codice del nuovo componente:");
  ...
  }, [selectedBomId, smartRefresh, projectArticlesActions, toast, setSelectedComponents]);
  ...
  const updateComponent = useCallback((line, data) => {
  ...
  const reorderBOMRoutings = useCallback(async (bomId, cycles) => {
    ...
    try {
      setLoading(true);
      const result = await projectArticlesActions.reorderBOMRoutings(bomId, cycles);
      ...
      toast({
        title: "Errore",
        description: error.message || "Si è verificato un errore durante il riordinamento dei cicli",
        variant: "destructive"
      });
      return { success: 0 };
    } finally {
