...
import { AlertCircle, Save, ListFilter, ShoppingCart, Package, Edit, Clock, FileSpreadsheet, Database, ArrowLeft } from 'lucide-react';
 ...
 */
const ArticleDetails = ({ item, project, canEdit, onBOMView, onRefresh, onBack }) => {
  // Stati locali
    ...
    const loadArticleDetails = async () => {
      ...
      try {
        ...
        if (data && typeof data === 'object' && 'Id' in data) {
          ...
          console.warn('Invalid or missing article data for ID:', item.Id, 'Data:', data);
        }
      } catch (error) {
        console.error('Error loading article details:', error);
        swal.fire({
          title: 'Errore',
          text: error.message || 'Si è verificato un errore nel caricamento dei dettagli articolo',
          icon: 'error',
        });
      } finally {
        setLoading(false);
  ...
  const handleSave = async () => {
    try {
      ...
      if (result && result.success) {
        ...
        throw new Error((result && result.msg) || 'Errore durante l\'aggiornamento');
      }
    } catch (error) {
      console.error('Error updating article:', error);
      swal.fire({
        title: 'Errore',
        text: error.message || 'Si è verificato un errore nell\'aggiornamento dell\'articolo',
        icon: 'error',
      });
    } finally {
      setLoading(false);
  ...
  const handleUnlinkFromProject = async () => {
    try {
      if (!project || !project.ProjectID || !articleData || !articleData.Id) {
        throw new Error('Dati insufficienti per rimuovere l\'articolo dal progetto');
      }

      const result = await unlinkItemFromProject(project.ProjectID, articleData.Id);
    ...
    <div className="h-full flex flex-col overflow-auto">
      ...
      >
        ...
        <div className="flex-1 overflow-auto">
          ...
          <TabsContent value="general" className="pt-2 px-6 h-full overflow-auto">
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  {isEditing ? (
                    ...
                    <div className="border rounded-md p-2 bg-slate-50">{articleData.Item}</div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label htmlFor="customerItemReference">Riferimento Cliente</Label>
                  {isEditing ? (
                    <Input
          ...
          {/* Tab Dimensioni */}
          <TabsContent value="dimensions" className="pt-2 px-6 h-full overflow-auto">
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-md p-4 bg-slate-50">
                  <div className="flex flex-wrap gap-3">
                    {articleData.Diameter ? (
                      <Badge className="bg-gray-200 text-gray-700">Ø {articleData.Diameter}</Badge>
                    ) : null}
                    {articleData.Bxh ? (
                      <Badge className="bg-gray-200 text-gray-700">{articleData.Bxh}</Badge>
                    ) : null}
                    {articleData.Depth ? (
                      <Badge className="bg-gray-200 text-gray-700">P {articleData.Depth}</Badge>
                    ) : null}
                    {articleData.Length ? (
                      <Badge className="bg-gray-200 text-gray-700">L {articleData.Length}</Badge>
                    ) : null}
                    {articleData.MediumRadius ? (
                      <Badge className="bg-gray-200 text-gray-700">R {articleData.MediumRadius}</Badge>
                    ) : null}
                    {!articleData.Diameter && !articleData.Bxh && !articleData.Depth && 
                     !articleData.Length && !articleData.MediumRadius && (
                      <span className="text-gray-400 text-sm">Nessuna dimensione specificata</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          <TabsContent value="projects" className="pt-2 px-6 h-full overflow-auto">
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  {articleData.projects && articleData.projects.length > 0 ? (
                    <div className="space-y-4">
                      {articleData.projects.map((proj) => (
                        <div key={proj.ProjectID} className="border rounded-md p-3 hover:bg-slate-50">
                          <div className="font-medium">{proj.ProjectName}</div>
                          <div className="flex items-center gap-3 mt-1">
                            <Badge className="bg-gray-100 text-gray-700">{proj.ProjectStatus}</Badge>
          ...
          <TabsContent value="references" className="pt-2 px-6 h-full overflow-auto">
            <ArticleReferences
              article={articleData}
              project={project}
              canEdit={canEdit}
