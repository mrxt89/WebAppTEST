...
const TaskHistoryTab = ({ task }) => {
  ...
  const getStatusBadgeColor = (status) => {
    switch (status) {
      ...
      case 'IN ESECUZIONE': return 'bg-blue-100 text-blue-800 border-blue-200';
      case 'BLOCCATA': return 'bg-red-100 text-red-800 border-red-200';
      case 'SOSPESA': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
  ...
  const renderChanges = (current, previous) => {
    ...
    if (current.StartDate !== previous?.StartDate) {
      changes.push({
        field: 'Data Inizio',
        oldValue: previous?.StartDate ? new Date(previous.StartDate).toLocaleDateString() : '',
    ...
    if (current.DueDate !== previous?.DueDate) {
      changes.push({
        field: 'Data Fine',
        oldValue: previous?.DueDate ? new Date(previous.DueDate).toLocaleDateString() : '',
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        <div className="flex-1">
                          ...
                          {change.isBadge ? (
                            ...
                            <div className="text-gray-700">{change.oldValue || 'Non impostato'}</div>
                          )}
                        </div>
                        <div className="flex-1">
                          ...
                          {change.isBadge ? (
                            ...
                            <div className="text-gray-700">{change.newValue}</div>
                          )}
                        </div>
                      </div>
                    </div>
