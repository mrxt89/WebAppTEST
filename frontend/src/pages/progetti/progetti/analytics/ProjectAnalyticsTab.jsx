...
import { 
  DollarSign, TrendingUp, Users, Calendar, PieChartIcon, 
...
const ProjectAnalyticsTab = ({ project, refreshProject }) => {
  ...
  const fetchWorkHoursData = async (projectId) => {
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/timetracking/projects/${projectId}/summary`, {
        headers: {
      ...
      try {
        ...
        for (const task of project.tasks) {
          ...
          try {
            ...
            console.error(`Error loading costs for task ${task.TaskID}:`, error);
          }
        }
    ...
    try {
      ...
      switch (activeTab) {
        case 'costs':
          ...
          csvContent = 'Categoria,Descrizione,Quantità,UoM,Costo Unitario,Costo Totale\n';
        ...
        case 'progress':
          ...
          csvContent = 'Titolo,Stato,Priorità,Assegnato a,Data Inizio,Data Fine,Giorni\n';
        ...
        case 'resources':
          ...
          csvContent = 'Risorsa,Attività Assegnate,Completate,In Corso,In Ritardo,Tasso Completamento,Ore Interne,Ore Esterne\n';
          ...
          if (workHoursData && workHoursData.length > 0) {
            workHoursData.forEach(entry => {
              ...
              if (entry.WorkType === 'INTERNO') {
                resourcesFromHours[userId].internalHours += parseFloat(entry.UserHours) || 0;
              } else if (entry.WorkType === 'ESTERNO') {
                resourcesFromHours[userId].externalHours += parseFloat(entry.UserHours) || 0;
              }
            });
          }
          ...
          project.tasks.forEach(task => {
            ...
            if (!workload[task.AssignedTo]) {
              workload[task.AssignedTo] = {
                ...
                inProgress: 0,
                delayed: 0,
                internalHours: resourcesFromHours[task.AssignedTo]?.internalHours || 0,
                externalHours: resourcesFromHours[task.AssignedTo]?.externalHours || 0
              };
            }
          ...
          Object.values(workload).forEach(resource => {
            const completionRate = resource.assigned > 0 
        ...
        case 'timeline':
          ...
          csvContent = 'Titolo,Data Inizio,Data Fine,Durata (giorni),Stato,Ore Lavorate\n';
          
          // Raggruppa le ore per attività
          const taskHours = {};
          if (workHoursData && workHoursData.length > 0) {
            project.tasks.forEach(task => {
              const taskEntries = workHoursData.filter(e => e.TaskID === task.TaskID);
              const totalHours = taskEntries.reduce((sum, entry) => sum + (parseFloat(entry.UserHours) || 0), 0);
      ...
      <div className="flex-grow flex flex-col overflow-hidden">
        ...
        <ScrollArea className="flex-grow p-6">
          {isLoading ? (
            ...
            <>
              <TabsContent value="costs" className="m-0 outline-none" forceMount={activeTab === "costs"}>
                <CostAnalysis 
              ...
              <TabsContent value="progress" className="m-0 outline-none" forceMount={activeTab === "progress"}>
                <ProgressAnalysis 
              ...
              <TabsContent value="resources" className="m-0 outline-none" forceMount={activeTab === "resources"}>
                <ResourceAnalysis 
              ...
              <TabsContent value="timeline" className="m-0 outline-none" forceMount={activeTab === "timeline"}>
                <TimeAnalysis 
