...
const TimeAnalysis = ({ project, loading, workHoursData = [] }) => {
      ...
      workHoursData.forEach((entry) => {
        ...
        if (entry.WorkDate) {
          ...
          if (tasksByMonth[workDateMonth]) {
            tasksByMonth[workDateMonth].hours +=
  ...
  const burndownData = useMemo(() => {
    ...
    const sortedTasks = [...tasks]
      ...
      .sort(
        (a, b) =>
          new Date(a.TBModified || a.TBCreated) -
          new Date(b.TBModified || b.TBCreated),
      );
    ...
    if (result.length > 1 && project.EndDate) {
      ...
      result.forEach((point, index) => {
        ...
        point.ideal = Math.max(
          0,
          Math.round(tasks.length - daysPassed * tasksPerDay),
        );
      });
  ...
  const workHoursByTask = useMemo(() => {
    ...
    workHoursData.forEach((entry) => {
      ...
      if (entry.WorkType === "INTERNO") {
        taskHours[entry.TaskID].internalHours +=
          parseFloat(entry.UserHours) || 0;
      } else if (entry.WorkType === "ESTERNO") {
        taskHours[entry.TaskID].externalHours +=
          parseFloat(entry.UserHours) || 0;
      }
    });
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <div className="text-3xl font-bold text-amber-600">
                {totalWorkHours.total.toFixed(1)}
              </div>
            ...
            {timelineData.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                ...
                >
                  ...
                  <YAxis
                    yAxisId="right"
                    orientation="right"
                    label={{ value: "Ore", angle: 90, position: "insideRight" }}
                  />
                  <RechartsTooltip />
                  <Legend />
                  <Bar
              ...
              <div className="flex flex-col items-center justify-center h-[300px] text-gray-500">
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Il progetto deve avere date di inizio e fine definite
                </p>
              </div>
            )}
          </CardContent>
                  ...
                  {totalWorkHours.total > 0 ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        >
                          {totalWorkHours.internal.toFixed(1)}h (
                          {(
                            (totalWorkHours.internal / totalWorkHours.total) *
                            100
                          ).toFixed(1)}
                          %)
                        </Badge>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <Badge
                          ...
                          className="bg-green-50 text-green-700 border-green-200"
                        >
                          {totalWorkHours.external.toFixed(1)}h (
                          {(
                            (totalWorkHours.external / totalWorkHours.total) *
                            100
                          ).toFixed(1)}
                          %)
                        </Badge>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <Badge
                          ...
                          className="bg-gray-50 text-gray-700"
                        >
                          {workHoursByTask.length} di {tasks.length} (
                          {Math.round(
                            (workHoursByTask.length / tasks.length) * 100,
                          )}
                          %)
                        </Badge>
                      </div>
                    ...
                    <div className="text-center p-6 bg-gray-50 rounded-lg text-gray-500">
                      ...
                      <p className="text-sm mt-1">
                        Utilizza il timesheet per registrare le ore
                      </p>
                    </div>
                  )}
                </div>
              ...
              return (
                ...
                >
                  <div className="space-y-1">
                    ...
                    <Progress
                      value={percentage}
                      className="h-2 mb-1 bg-gray-100"
