...
import { BarChart, Bar, XAxis, YAxis, PieChart, Pie, 
...
const ResourceAnalysis = ({ project, loading, workHoursData = [] }) => {
  ...
  const workloadByPerson = useMemo(() => {
    ...
    return Object.values(workload)
      .filter(person => person.assigned > 0 || person.internalHours > 0 || person.externalHours > 0)
      .map(person => ({
        ...person,
        completionRate: person.assigned > 0 
  ...
  const workloadChartData = useMemo(() => {
    return workloadByPerson.map(person => ({
      name: person.name,
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100">
          <CardContent className="pt-6">
            <div className="text-center">
              <h4 className="text-blue-700 mb-2 font-medium">Persone Coinvolte</h4>
        ...
        <Card className="bg-gradient-to-br from-emerald-50 to-emerald-100">
          <CardContent className="pt-6">
            <div className="text-center">
              <h4 className="text-emerald-700 mb-2 font-medium">Tasso Medio Completamento</h4>
              <div className="text-3xl font-bold text-emerald-600">
                {workloadByPerson.length > 0 
                  ? Math.round(
                      workloadByPerson.reduce((sum, p) => sum + p.completionRate, 0) / workloadByPerson.length
                    )
                  : 0}%
              </div>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100">
          <CardContent className="pt-6">
            <div className="text-center">
              <h4 className="text-purple-700 mb-2 font-medium">Attività in Corso</h4>
        ...
        <Card className="bg-gradient-to-br from-amber-50 to-amber-100">
          <CardContent className="pt-6">
            <div className="text-center">
              <h4 className="text-amber-700 mb-2 font-medium">Ore Totali Registrate</h4>
              <div className="text-3xl font-bold text-amber-600">
                {workloadByPerson.reduce((sum, p) => sum + p.totalHours, 0).toFixed(1)}h
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-gray-500">Distribuzione del Carico di Lavoro</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {workloadChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height={350} minHeight={350}>
                  ...
                  >
                    ...
                    <Bar name="Completate" dataKey="completed" stackId="a" fill="#10b981" />
                    <Bar name="In Corso" dataKey="inProgress" stackId="a" fill="#3b82f6" />
                    <Bar name="In Ritardo" dataKey="delayed" stackId="a" fill="#ef4444" />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="flex flex-col items-center justify-center h-[350px] text-gray-500">
                  ...
                  <p className="text-gray-500">Nessun dato sul carico di lavoro</p>
                  <p className="text-sm text-gray-400 mt-1">Assegna delle attività per visualizzare le statistiche</p>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-gray-500">Top Performers</CardTitle>
          </CardHeader>
          <CardContent>
            {topPerformers.length > 0 ? (
                ...
                {topPerformers.map((person, index) => (
                  <div key={person.id} className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge 
                        className={`${
                          person.completionRate >= 75 ? 'bg-green-100 text-green-700' : 
                          person.completionRate >= 50 ? 'bg-blue-100 text-blue-700' : 
                    ...
                    <Progress 
                      ...
                      indicatorClassName={
                        person.completionRate >= 75 ? 'bg-green-500' : 
                        person.completionRate >= 50 ? 'bg-blue-500' : 
                        'bg-amber-500'
                      } 
                    />
                ...
                <div className="pt-4 border-t">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="p-3 bg-gray-50 rounded-lg">
                      <div className="text-sm text-gray-500 mb-1">Task Completate</div>
                      <div className="text-xl font-semibold text-green-600">
                        {tasks.filter(t => t.Status === 'COMPLETATA').length}
                      </div>
                    </div>
                    <div className="p-3 bg-gray-50 rounded-lg">
                      <div className="text-sm text-gray-500 mb-1">Task Pendenti</div>
              ...
              <div className="flex flex-col items-center justify-center h-[350px] text-gray-500">
                ...
                <p className="text-sm text-gray-400 mt-1">Completare più attività per visualizzare i top performer</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm text-gray-500">Dettaglio Performance</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-2 px-4 text-sm font-medium text-gray-500">Persona</th>
                    <th className="text-center py-2 px-4 text-sm font-medium text-gray-500">Assegnate</th>
                    <th className="text-center py-2 px-4 text-sm font-medium text-gray-500">Completate</th>
                    <th className="text-center py-2 px-4 text-sm font-medium text-gray-500">In Corso</th>
                    <th className="text-center py-2 px-4 text-sm font-medium text-gray-500">In Ritardo</th>
                    <th className="text-center py-2 px-4 text-sm font-medium text-gray-500">Completamento</th>
                    <th className="text-center py-2 px-4 text-sm font-medium text-gray-500">Ore Interne</th>
                    <th className="text-center py-2 px-4 text-sm font-medium text-gray-500">Ore Esterne</th>
                  </tr>
                </thead>
                <tbody>
                  {workloadByPerson.length === 0 ? (
                    ...
                    workloadByPerson.map((person) => (
                      <tr key={person.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="text-center py-3 px-4">{person.assigned}</td>
                        ...
                        <td className="text-center py-3 px-4">
                          <Badge 
                            variant={person.delayed > 0 ? "destructive" : "outline"}
                            className={person.delayed > 0 ? "" : "text-gray-400"}
                          >
                            {person.delayed}
                          </Badge>
                        </td>
                        <td className="text-center py-3 px-4">
                          <Badge 
                            variant="outline" 
                            className={`
                              ${person.completionRate > 75 ? 'bg-green-100 text-green-700 border-green-200' : 
                                person.completionRate > 50 ? 'bg-blue-100 text-blue-700 border-blue-200' : 
