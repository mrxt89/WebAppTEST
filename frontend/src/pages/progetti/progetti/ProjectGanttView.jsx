...
import {
  Search, Filter, AlertTriangle, ZoomIn, ZoomOut,
  Calendar, ArrowUp, X, ArrowDown, MoveVertical
} from 'lucide-react';
...
const ProjectGanttView = ({ 
  ...
  useEffect(() => {
    renderCount.current++;
    console.log(`[GANTT] Render #${renderCount.current} (Normal React lifecycle)`);
    return () => {
      console.log(`[GANTT] Component unmounted after render #${renderCount.current}`);
    };
  }, []);
  ...
  const handleTaskChangeWrapper = async (task) => {
    ...
    if (!checkAdminPermission(project) && !isOwnTask(originalTask)) {
      swal.fire('Attenzione', 'Non hai i permessi per modificare questa attività', 'warning');
      return;
    }
    
    // No modifications for completed tasks
    if (originalTask.Status === 'COMPLETATA') {
      swal.fire('Attenzione', 'Non puoi modificare un\'attività completata', 'warning');
      return;
    }
  ...
  const convertTasks = useCallback((sourceTasks) => {
    return sourceTasks.map(task => ({
      id: task.TaskID.toString(),
      name: task.Title,
      start: new Date(task.StartDate),
      end: new Date(task.DueDate),
      progress: task.Status === 'COMPLETATA' ? 100 : 
               task.Status === 'IN ESECUZIONE' ? 50 : 
               task.Status === 'BLOCCATA' ? 0 : 
               task.Status === 'SOSPESA' ? 25 : 
               10, // Per "DA FARE"
      type: 'task',
      project: project.ProjectID.toString(),
      dependencies: task.PredecessorTaskID ? [task.PredecessorTaskID.toString()] : [],
      hideChildren: false,
      ...
      isDelayed: new Date(task.DueDate) < new Date() && task.Status !== 'COMPLETATA',
      assignedToName: task.AssignedToName,
      // Stile in base allo stato
      styles: {
        backgroundColor: task.Status === 'COMPLETATA' ? '#10b981' :
                         task.Status === 'IN ESECUZIONE' ? '#3b82f6' :
                         task.Status === 'BLOCCATA' ? '#ef4444' :
                         task.Status === 'SOSPESA' ? '#f59e0b' :
                         '#94a3b8',
        progressColor: task.Status === 'COMPLETATA' ? '#10b981' :
                       task.Status === 'IN ESECUZIONE' ? '#3b82f6' :
                       task.Status === 'BLOCCATA' ? '#ef4444' :
                       task.Status === 'SOSPESA' ? '#f59e0b' :
                       '#94a3b8',
        progressSelectedColor: task.Status === 'COMPLETATA' ? '#10b981' :
                       task.Status === 'IN ESECUZIONE' ? '#3b82f6' :
                       task.Status === 'BLOCCATA' ? '#ef4444' :
                       task.Status === 'SOSPESA' ? '#f59e0b' :
                       '#94a3b8',
  ...
  const executeTaskSequenceUpdate = async (taskId, projectId, newIndex) => {
    try {
      console.log('[GANTT] Esecuzione della chiamata updateTaskSequence:', taskId, projectId, newIndex);
  ...
  const handleTaskMove = async (task, orderIndex) => {
    ...
    clickCount.current++;
    console.log(`[GANTT] handleTaskMove #${clickCount.current} chiamato con:`, task, orderIndex);
    ...
    try {
      ...
      if (!checkAdminPermission(project) && !isOwnTask(originalTask)) {
        ...
        swal.fire('Attenzione', 'Non hai i permessi per spostare questa attività', 'warning');
        moveInProgress.current = false;
        clickHandledRef.current = false;
        return;
      }
      ...
      console.log('[GANTT] Chiamata diretta executeTaskSequenceUpdate con:', taskId, project.ProjectID, orderIndex);
      
      // Usa la funzione diretta invece di quella debounced
      const result = await executeTaskSequenceUpdate(taskId, project.ProjectID, orderIndex);
      ...
      swal.fire('Errore', 'Non è stato possibile aggiornare la sequenza delle attività', 'error');
    } finally {
      // Ripristina i flag dopo un ritardo
      setTimeout(() => {
        moveInProgress.current = false;
        clickHandledRef.current = false;
        console.log('[GANTT] Operazione di spostamento completata, flag ripristinati');
      }, 800);
    }
  };
    ...
    return (
      <Card className="border rounded-lg bg-white">
        <CardContent className="p-6 text-center">
          <div className="text-gray-500">Nessuna attività presente nel progetto</div>
        </CardContent>
      </Card>
    );
  }
  ...
  const handleMoveButtonClick = (e, task, direction) => {
    ...
    if (moveInProgress.current || clickHandledRef.current) {
      console.log('[GANTT] Operazione di spostamento già in corso, ignoro la richiesta');
      return;
    }
    ...
    setTimeout(() => {
      ...
      const tasks = ganttTasks.map(t => parseInt(t.originalTask.TaskSequence));
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 p-2">
            ...
            <div className="flex-grow-0 flex-shrink-0 w-auto">
              ...
              >
                ...
                <SelectContent>
                  ...
                  {users
                    .filter(user => tasks.some(task => task.AssignedTo === user.userId))
                    .map(user => (
          ...
          <div className="flex items-center gap-2 ">
            <Button
              ...
              onClick={() => setFilters(prev => ({ ...prev, showDelayed: !prev.showDelayed }))}
              className={filters.showDelayed ? "bg-amber-600 hover:bg-amber-700" : ""}
            >
        ...
        >
          {ganttTasks.length > 0 ? (
            <Gantt
              ...
              TooltipContent={({ task }) => {
                ...
                return (
                  <div className="p-2 bg-white shadow-lg rounded border">
                    ...
                    <div>Inizio: {new Date(task.start).toLocaleDateString()}</div>
                    ...
                    {originalTask.Description && (
                      <div className="mt-1 pt-1 border-t">
                        <div className="text-xs text-gray-600">Descrizione:</div>
                  ...
                  <table className="w-full h-full border-spacing-0 border-separate">
                    <tbody>
                      {ganttTasks.map((task, index) => {
                        ...
                        const canMove = checkAdminPermission(project) || isOwnTask(task.originalTask);
                                ...
                                <div className="flex flex-col gap-1 items-center">
                                  ...
                                  <button 
                                    ...
                                    className={`p-1 rounded bg-white hover:bg-gray-200 border border-gray-300 ${
                                      index === 0 || moveInProgress.current || clickHandledRef.current ? 
                                      'opacity-30 cursor-not-allowed' : 
                                      'hover:border-gray-400'
                                    }`}
                                    disabled={index === 0 || moveInProgress.current || clickHandledRef.current}
                                    title="Sposta su"
                                  >
                                  ...
                                  <button 
                                    ...
                                    className={`p-1 rounded bg-white hover:bg-gray-200 border border-gray-300 ${
                                      index === ganttTasks.length - 1 || moveInProgress.current || clickHandledRef.current ? 
                                      'opacity-30 cursor-not-allowed' : 
                                      'hover:border-gray-400'
                                    }`}
                                    disabled={index === ganttTasks.length - 1 || moveInProgress.current || clickHandledRef.current}
                                    title="Sposta giù"
                                  >
              ...
              columnWidth={viewMode === ViewMode.Month ? 300 : viewMode === ViewMode.Week ? 170 : 60}
              listCellWidth="100px"
...
export default React.memo(ProjectGanttView, (prevProps, nextProps) => {
  ...
  const tasksUnchanged = 
    ...
    JSON.stringify(prevProps.tasks.map(t => t.TaskID + '-' + t.TaskSequence)) === 
    JSON.stringify(nextProps.tasks.map(t => t.TaskID + '-' + t.TaskSequence));

  const projectUnchanged = prevProps.project.ProjectID === nextProps.project.ProjectID;
