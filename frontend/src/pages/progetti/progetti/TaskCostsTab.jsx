...
const CostDialog = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.qty || parseFloat(formData.qty) <= 0)
      newErrors.qty = "Inserisci una quantità valida";
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>
            {initialData ? "Modifica Costo" : "Nuovo Costo"}
          </DialogTitle>
        </DialogHeader>
              ...
              <Label className="flex items-center gap-2">
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>
                        Seleziona la categoria che meglio descrive questo costo
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
...
const TaskCostsTab = ({ task, canEdit, onCostChange }) => {
  ...
  useEffect(() => {
    ...
    if (task?.Costs) {
      try {
        const initialCosts =
          typeof task.Costs === "string" ? JSON.parse(task.Costs) : task.Costs;
