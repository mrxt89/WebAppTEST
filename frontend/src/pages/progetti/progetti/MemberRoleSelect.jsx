...
const MemberRoleSelect = ({ member, onRoleUpdate, disabled = false }) => {
  ...
  const handleRoleChange = async (newRole) => {
    ...
    try {
      ...
      toast({
        title: "Errore",
        description: "Si è verificato un errore durante l'aggiornamento del ruolo",
        variant: "destructive",
        duration: 3000,
      });
    } finally {
    ...
    >
      ...
      <SelectContent>
        <SelectItem value="ADMIN" className="flex items-center">
          <div className="flex items-center">
            {role === 'ADMIN' && <Check className="h-4 w-4 mr-2 text-green-500" />}
            <span className={role === 'ADMIN' ? 'font-medium' : ''}>Admin</span>
          </div>
        </SelectItem>
        <SelectItem value="MANAGER" className="flex items-center">
          <div className="flex items-center">
            {role === 'MANAGER' && <Check className="h-4 w-4 mr-2 text-green-500" />}
            <span className={role === 'MANAGER' ? 'font-medium' : ''}>Manager</span>
          </div>
        </SelectItem>
        <SelectItem value="USER" className="flex items-center">
          <div className="flex items-center">
            {role === 'USER' && <Check className="h-4 w-4 mr-2 text-green-500" />}
