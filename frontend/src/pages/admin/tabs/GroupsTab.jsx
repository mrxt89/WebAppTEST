...
const GroupsTab = ({ groups, users, updateGroup, assignUserToGroup, removeUserFromGroup, refreshData }) => {
    ...
    swal.fire({
      ...
      if (result.isConfirmed) {
        updateGroup(result.value.groupId, result.value)
          .then(() => {
            swal.fire('Successo', 'Gruppo aggiornato con successo.', 'success');
            refreshData('groups');
          })
          .catch((error) => {
            console.error('Errore durante l\'aggiornamento del gruppo:', error);
            swal.fire('Errore', error.response.data || 'Errore durante l\'aggiornamento del gruppo.', 'error');
          });
      }
    });
  };
  ...
  const handleAssignUsersToGroup = () => {
    if (selectedGroup && selectedUsers.length > 0) {
      Promise.all(selectedUsers.map(userId => assignUserToGroup(userId, selectedGroup.groupId)))
        .then(() => {
          swal.fire('Successo', 'Utenti aggiunti al gruppo con successo.', 'success');
            refreshData('groups'); // Aggiorna i gruppi
            setSelectedUsers([]); // Reset selezione
        })
        .catch((error) => {
          console.error('Errore durante l\'aggiunta degli utenti al gruppo:', error);
          swal.fire('Errore', 'Errore durante l\'aggiunta degli utenti al gruppo.', 'error');
        });
    }
  };
  ...
  const handleRemoveUsersFromGroup = () => {
    if (selectedGroup && selectedUsers.length > 0) {
      Promise.all(selectedUsers.map(userId => removeUserFromGroup(userId, selectedGroup.groupId)))
        .then(() => {
          swal.fire('Successo', 'Utenti rimossi dal gruppo con successo.', 'success');
            refreshData('groups'); // Aggiorna i gruppi
          setSelectedUsers([]); // Reset selezione
        })
        .catch((error) => {
          console.error('Errore durante la rimozione degli utenti dal gruppo:', error);
          swal.fire('Errore', 'Errore durante la rimozione degli utenti dal gruppo.', 'error');
        });
    }
  };
  ...
  const renderGroupDetails = () => {
    if (!selectedGroup) {
      return (
        <div className="flex items-center justify-center h-full">
          <p className="text-muted-foreground">Seleziona un gruppo per visualizzare i dettagli</p>
        </div>
      );
    }
    ...
    const availableUsers = users.filter(user => !groupUsers.some(groupUser => groupUser.userId === user.userId));
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex justify-between items-center mb-2 h-14">
            ...
            {selectedUsers.length > 0 && (
              <Button variant="destructive" size="sm" onClick={handleRemoveUsersFromGroup}>
          ...
          <ScrollArea className="h-60 border rounded-md p-2">
            {groupUsers.length > 0 ? (
              <div className="space-y-2">
                {groupUsers.map(user => (
                  <div key={user.userId} className="flex items-center space-x-2 p-2 hover:bg-accent rounded">
                    <div className="flex items-center space-x-2 w-full">
                      ...
                      />
                      <Label htmlFor={`group-user-${user.userId}`} className="flex-1 cursor-pointer">
              ...
              <p className="text-muted-foreground p-2">Nessun utente nel gruppo</p>
            )}
          </ScrollArea>
        </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-2 h-14">
            ...
            {selectedUsers.length > 0 && (
              <Button variant="default" size="sm" onClick={handleAssignUsersToGroup}>
                  ...
                  <div key={user.userId} className="flex items-center space-x-2 p-2 hover:bg-accent rounded">
                    <div className="flex items-center space-x-2 w-full">
                      <Checkbox 
                        ...
                        onCheckedChange={() => handleUserCheckbox(user.userId)}
                      />
                      <Label htmlFor={`available-user-${user.userId}`} className="flex-1 cursor-pointer">
                        {user.firstName} {user.lastName} ({user.username})
                      </Label>
                    </div>
                  </div>
