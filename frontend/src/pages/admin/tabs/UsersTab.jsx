...
const UsersTab = ({ 
  ...
  const handleEditUser = (user) => {
    swal.fire({
      ...
      preConfirm: () => {
        ...
        return { userId: user.userId, firstName, lastName, email, phoneNumber, role };
      }
    }).then((result) => {
      if (result.isConfirmed) {
        updateUser(result.value.userId, result.value)
          ...
          .catch((error) => {
            ...
            swal.fire('Errore', error.response?.data || 'Errore durante l\'aggiornamento dell\'utente.', 'error');
          });
      }
    });
  };
      ...
      if (result.isConfirmed) {
        resetPassword(result.value.userId, result.value.newPassword)
          ...
          .catch((error) => {
            ...
            swal.fire('Errore', error.response?.data || 'Errore durante il reset della password.', 'error');
          });
      }
    });
  };
  ...
  const handleToggleUserStatus = (userId, currentStatus) => {
    ...
    toggleUserStatus(userId, newStatus)
      .then(() => {
        swal.fire('Successo', `Utente ${currentStatus ? 'riattivato' : 'disabilitato'} con successo.`, 'success');
        refreshData('users');
      ...
      .catch((error) => {
        console.error(`Errore durante ${currentStatus ? 'la riattivazione' : 'la disabilitazione'} dell'utente:`, error);
        swal.fire('Errore', error.response?.data || `Errore durante ${currentStatus ? 'la riattivazione' : 'la disabilitazione'} dell'utente.`, 'error');
      });
  };
  ...
  const handleAssignCompaniesToUser = () => {
    if (selectedUser && selectedCompanies.length > 0) {
      Promise.all(selectedCompanies.map(companyId => 
        assignUserToCompany(selectedUser.userId, companyId)
      ))
        .then(() => {
          swal.fire('Successo', 'Aziende assegnate all\'utente con successo.', 'success');
          handleSelectUser(selectedUser); // Ricarica i dati dell'utente
          setSelectedCompanies([]); // Reset selezione
        })
        .catch((error) => {
          console.error('Errore durante l\'assegnazione delle aziende all\'utente:', error);
          swal.fire('Errore', 'Errore durante l\'assegnazione delle aziende all\'utente.', 'error');
        });
    }
  };
      ...
      Promise.all(selectedCompanies.map(companyId => 
        ...
        .then((results) => {
          ...
          if (errors.length > 0) {
            ...
            swal.fire('Successo', 'Aziende rimosse dall\'utente con successo.', 'success');
          }
        ...
        .catch((error) => {
          console.error('Errore durante la rimozione delle aziende dall\'utente:', error);
          swal.fire('Errore', 'Errore durante la rimozione delle aziende dall\'utente.', 'error');
        });
    }
  };
  ...
  const handleSetPrimaryCompany = (companyId) => {
    if (selectedUser) {
      setPrimaryCompany(selectedUser.userId, companyId)
        .then(() => {
          swal.fire('Successo', 'Azienda principale impostata con successo.', 'success');
          handleSelectUser({
            ...selectedUser,
            CompanyId: companyId
        ...
        .catch((error) => {
          console.error('Errore durante l\'impostazione dell\'azienda principale:', error);
          swal.fire('Errore', 'Errore durante l\'impostazione dell\'azienda principale.', 'error');
        });
    }
  };
    ...
    if (!selectedUser) {
      return (
        <div className="flex items-center justify-center h-full">
          <p className="text-muted-foreground">Seleziona un utente per visualizzare i dettagli</p>
        </div>
      );
    }

    // Trova l'azienda principale dell'utente
    const primaryCompany = companies.find(company => company.CompanyId === selectedUser.CompanyId);

    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between h-14">
          <h3 className="text-lg font-medium">{selectedUser.firstName} {selectedUser.lastName}</h3>
          <Badge variant={selectedUser.userDisabled ? "destructive" : "success"}>
        ...
        <div className="flex space-x-2">
          ...
          <Button variant="outline" onClick={() => handleResetPassword(selectedUser.userId)}>
            <i className="fa-solid fa-key mr-2"></i>
            Reset password
          </Button>
          <Button 
            variant={selectedUser.userDisabled ? "default" : "destructive"} 
            onClick={() => handleToggleUserStatus(selectedUser.userId, selectedUser.userDisabled)}
          >
        ...
        <div>
          <div className="flex justify-between items-center mb-2 h-14">
            ...
            {selectedCompanies.length > 0 && (
              <Button variant="destructive" size="sm" onClick={handleRemoveCompaniesFromUser}>
              ...
              {userCompanies.length > 0 ? (
                <div className="space-y-2">
                  {userCompanies.map(company => (
                    <div key={company.CompanyId} className="flex items-center space-x-2 p-2 hover:bg-accent rounded">
                      <div className="flex items-center space-x-2 w-full">
                        <Checkbox 
                          ...
                          onCheckedChange={() => handleCompanyCheckbox(company.CompanyId)}
                        />
                        <Label htmlFor={`company-${company.CompanyId}`} className="flex-1 cursor-pointer">
                          {company.Description} {company.CompanyId === selectedUser.CompanyId && 
                ...
                <p className="text-muted-foreground p-2">Nessuna azienda associata</p>
              )}
            </ScrollArea>
          )}
        </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-2 h-14">
            ...
            {selectedCompanies.length > 0 && (
              <Button variant="default" size="sm" onClick={handleAssignCompaniesToUser}>
          ...
          <ScrollArea className="h-40 border rounded-md p-2">
            {companies.length > 0 ? (
              <div className="space-y-2">
                {companies
                  .filter(company => !userCompanies.some(uc => uc.CompanyId === company.CompanyId))
                  .map(company => (
                    <div key={company.CompanyId} className="flex items-center space-x-2 p-2 hover:bg-accent rounded">
                      <div className="flex items-center space-x-2 w-full">
                        <Checkbox 
                          ...
                          onCheckedChange={() => handleCompanyCheckbox(company.CompanyId)}
                        />
                        <Label htmlFor={`available-company-${company.CompanyId}`} className="flex-1 cursor-pointer">
                          {company.Description}
                        </Label>
                      </div>
                    </div>
              ...
              <p className="text-muted-foreground p-2">Nessuna azienda disponibile</p>
            )}
          </ScrollArea>
        </div>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="hidden md:table-cell">Cognome</TableHead>
                ...
                {Array.isArray(users) && users.map(user => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="hidden md:table-cell">{company?.Description || "-"}</TableCell>
                    </TableRow>
                  );
                })}
              </TableBody>
