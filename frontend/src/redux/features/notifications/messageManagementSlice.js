    ...
    try {
      ...
      if (response.data && response.data.Success) {
        ...
        return rejectWithValue(
          response.data?.Message || "Error editing message",
        );
      }
    } catch (error) {
  ...
  async (messageId, { rejectWithValue }) => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        return rejectWithValue(
          response.data?.message || "Error retrieving version history",
        );
      }
    } catch (error) {
      console.error("Error fetching message versions:", error);
      return rejectWithValue(
        error.message || "Failed to retrieve version history",
      );
    }
  },
);
  ...
  async ({ messageId, color }, { rejectWithValue, dispatch, getState }) => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        if (state.notifications && state.notifications.notifications) {
          for (const notification of state.notifications.notifications) {
            ...
            if (
              ...
              messages.some((msg) => msg.messageId === messageId)
            ) {
        ...
        return rejectWithValue(
          response.data?.message || "Failed to set message color",
        );
      }
    } catch (error) {
  ...
  async (messageId, { rejectWithValue, dispatch, getState }) => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        return rejectWithValue(
          response.data?.message || "Failed to clear message color",
        );
      }
    } catch (error) {
...
export const filterMessages = createAsyncThunk(
  ...
  async (
    { notificationId, color, searchText, messageType },
    { rejectWithValue },
  ) => {
    try {
      if (!notificationId) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(filterMessages.fulfilled, (state, action) => {
        state.loading = false;
        const { notificationId, filteredMessages, totalFound, filter } =
          action.payload;
