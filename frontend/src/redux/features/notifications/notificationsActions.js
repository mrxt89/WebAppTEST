          ...
          batch.map(async (notificationId) => {
            try {
              const attachments = await dispatch(
                fetchNotificationAttachments(notificationId),
              ).unwrap();
              results[notificationId] = attachments;
            } catch (err) {
              console.warn(
                `Error fetching attachments for notification ${notificationId}:`,
                err,
              );
            }
          }),
...
export const uploadNotificationAttachment = createAsyncThunk(
  ...
  async ({ notificationId, file, messageId = null }, { dispatch }) => {
    console.log("uploadNotificationAttachment called", {
      notificationId,
      file,
    }); // Sposta qui
    try {
    ...
    try {
      ...
      if (!res.data.success) {
        return rejectWithValue(
          res.data.message || "Failed to remove user from chat",
        );
      }
      ...
      document.dispatchEvent(
        new CustomEvent("user-removed-from-chat", {
          detail: {
            notificationId,
      ...
      return rejectWithValue(
        error.message || "Failed to remove user from chat",
      );
    }
  },
);
