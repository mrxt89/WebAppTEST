  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (error.response && (error.response.status === 401 || error.response.status === 403)) {
        ...
        return rejectWithValue({ type: 'auth_error', message: 'Session expired' });
      }
...
export const fetchNotificationById = createAsyncThunk(
  ...
  async (notificationId, { rejectWithValue, getState }) => {
    try {
      ...
      console.error(`Errore in fetchNotificationById per ${notificationId}:`, error);
...
export const createDBNotificationsView = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(`${config.API_BASE_URL}/DBNotificationsView`, {
    ...
    builder
      ...
      .addCase(sendNotification.fulfilled, (state, action) => {
        try {
          ...
          try {
            ...
            console.error('Errore nella ricerca della notifica esistente:', findError);
          }
          
          // Determine if user has left the chat
          const chatLeft = existingNotification ? existingNotification.chatLeft : 0;
          ...
          if (chatLeft && messages) {
            try {
              ...
              const filteredMessages = filterMessagesAfterLeaving(messagesArray, chatLeft);
          ...
          try {
            ...
            console.error('Errore nella ricerca dell\'indice della notifica:', findIndexError);
          }
          ...
          if (notificationIndex !== -1) {
            ...
            try {
              ...
              console.error('Errore nell\'aggiornamento della notifica esistente:', updateError);
            }
          } else {
            ...
            try {
              ...
              console.error('Errore nell\'aggiunta della nuova notifica:', pushError);
            }
          }
      ...
      .addCase('notifications/updateFromWorker', (state, action) => {
        try {
          ...
          if (!action.payload) {
            console.error('Payload non valido in updateFromWorker (null/undefined)');
            return;
          }
          
          if (!Array.isArray(action.payload)) {
            console.error('Payload non valido in updateFromWorker (non è un array):', action.payload);
            return;
          }
          ...
          if (unreadCountChanged) {
            ...
            try {
              ...
              console.error('Errore nell\'emissione dell\'evento notifications-updated:', eventError);
            }
          }
        } catch (error) {
...
const filterMessagesAfterLeaving = (messages, chatLeft) => {
  ...
  const leaveIndex = messages.findIndex(msg => 
    msg.message && msg.message.includes('ha lasciato la chat') && 
