...
export const fetchMessageReactions = createAsyncThunk(
  ...
  async (messageId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || 'Failed to fetch message reactions');
    }
  }
);
  ...
  async (messageIds = [], { rejectWithValue }) => {
    try {
      ...
      try {
        ...
        console.log('Batch reactions endpoint not available, falling back to separate requests');
      }
  ...
  async ({ messageId, reactionType }, { rejectWithValue, dispatch }) => {
    try {
      ...
      try {
        ...
        console.warn('Could not pre-fetch notificationId, will try to extract from response', err);
      }
      ...
      if (!response.data || !response.data.success) {
        return rejectWithValue(response.data?.message || 'Failed to toggle reaction');
      }
...
export const removeMessageReaction = createAsyncThunk(
  ...
  async (reactionId, { rejectWithValue, dispatch }) => {
    try {
      ...
      if (!response.data || !response.data.success) {
        return rejectWithValue(response.data?.message || 'Failed to remove reaction');
      }
...
export const selectReactionsLoading = (state) => 
  state.messageReactions.loading;
export const selectReactionsError = (state) => 
  state.messageReactions.error;
