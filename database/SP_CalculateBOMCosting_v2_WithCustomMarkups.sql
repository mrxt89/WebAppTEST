-- =============================================
-- SP_CalculateBOMCosting v2 - CON RICARICHI CUSTOM BOM
-- Versione modificata per usare ricarichi custom BOM-specific
-- =============================================
...
BEGIN
    ...
    -- ========================================
    -- NUOVO: Carica ricarichi effettivi (custom o globali)
    -- ========================================
    ...
    -- Variabili per i ricarichi effettivi (convertiti da % a decimale, es: 25 -> 0.25)
    ...
    SET @EffectiveRicaricoOPE = ISNULL(@EffectiveRicaricoOPE, 0.15);
    SET @EffectiveRicaricoTrasporto = ISNULL(@EffectiveRicaricoTrasporto, 0.02);
    SET @EffectiveRicaricoScarto = ISNULL(@EffectiveRicaricoScarto, 0.04);
    SET @EffectiveRicaricoTotale = ISNULL(@EffectiveRicaricoTotale, 0.20);
    ...
    -- Log inizio calcolo CON INFO RICARICHI
    DECLARE @LogMessage NVARCHAR(MAX) = 'OrderQty: ' + ISNULL(CAST(@OrderQuantity AS NVARCHAR(50)), '0') +
                                        ...
                                        (SELECT Source FROM #EffectiveMarkups WHERE ParameterName = 'MARKUP_RM') + ')';
    ...
    -- [... RESTO DEL CODICE ESPLOSIONE BOM INVARIATO ...]
    ...
    -- [... CALCOLO COSTI MATERIALI E OPERAZIONI INVARIATO ...]
    ...
    -- ========================================
    -- MODIFICATO: USA RICARICHI EFFETTIVI
    -- ========================================
    ...
    -- [... OUTPUT DEBUG INVARIATO ...]
    IF @Debug = 1
    BEGIN
        SELECT comp.ComponentId, comp.Line as ComponentLine, comp.ComponentId as ItemId,
            itm.Item as ItemCode, itm.Description as ItemDescription, comp.Quantity, comp.UoM,
            comp.UnitCost, comp.FixedCost, comp.TotalCost, comp.ComponentType,
...
PRINT '✅ SP_CalculateBOMCosting aggiornata con ricarichi custom BOM-specific!';
PRINT '';
PRINT 'Logica implementata:';
PRINT '1. Carica ricarichi custom per BOMId (se esistono)';
PRINT '2. Se non esistono custom → usa ricarichi globali';
PRINT '3. Applica ricarichi effettivi nel calcolo';
PRINT '';
PRINT 'Test: EXEC SP_CalculateBOMCosting @CompanyId = 1, @BOMId = 18822, @Debug = 1;';
GO
