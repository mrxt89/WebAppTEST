-- =============================================
-- 03 - Create SP MA_ApproveIntercompanyReference
-- Gestisce l'approvazione di un componente Intercompany e crea/aggiorna il progetto target
-- Author: Claude Code
-- Date: 2025-10-25
-- Updated: 2025-10-25 - Fix SCOPE_IDENTITY() → usa tabella temporanea per catturare ProjectID
-- =============================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            DECLARE @SPErrorMessage NVARCHAR(4000);
            ...
            -- SCOPE_IDENTITY() non funziona quando si chiama un'altra stored procedure
        ...
        -- =================================================================
        -- Gli allegati dell'articolo Intercompany vengono copiati come allegati del progetto target
        -- mantenendo lo STESSO FilePath (nessuna duplicazione fisica del file)
        ...
        INSERT INTO MA_ProjectAttachments (
            ProjectID,
            FileName,
            FilePath,       -- ⭐ Stesso FilePath dell'allegato originale
            FileType,
            FileSizeKB,
            UploadedBy,
            UploadedAt
        ...
        SELECT
            @TargetProjectId,
            ia.FileName,
            ia.FilePath,    -- ⭐ Punta allo stesso file fisico
            ia.FileType,
            ia.FileSizeKB,
            ia.UploadedBy,
            GETDATE()       -- Data di "copia" nel progetto
        ...
        WHERE ias.SourceProjectItemId = @SourceProjectItemId
          ...
          AND ia.IsVisible = 1  -- Solo allegati visibili
          -- Evita duplicati: se l'allegato è già nel progetto target, non copiarlo
          AND NOT EXISTS (
              SELECT 1
        ...
        SET
            ...
            ResponseNotes = ISNULL(@ResponseNotes, N'Richiesta approvata automaticamente'),
...
PRINT 'Stored Procedure MA_ApproveIntercompanyReference creata con successo (FIX: tabella temporanea per ProjectID)';
GO
